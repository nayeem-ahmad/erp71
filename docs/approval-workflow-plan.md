...
|---|---|---|---|---|
| Voucher | `schema.prisma:3454` | `approval_status`, `approved_by`, `approved_at`, `rejection_reason` | `APPROVE_VOUCHER` | tenant-wide on/off flag, anyone holding the permission |
| Leave request | `schema.prisma:5245` | `status`, `approvals_given`, `LeaveRequestApproval[]` (`:5284`) | `MANAGE_HR` | N levels from `LeaveType.approval_levels` (`:4533`), anyone holding the permission |
| Expense claim | `schema.prisma:4846` | `status`, `approved_by`, `approved_at`, `approver_note` | `MANAGE_HR` | single approver |
| Warehouse transfer | `schema.prisma:1711` | `requires_approval`, `approved_by`, `approval_date` | `APPROVE_GOODS_TRANSFER` | single approver |
| Product demand | `schema.prisma:1844` | `reviewed_by`, `reviewed_at`, per-line `quantity_approved` | `APPROVE_PRODUCT_DEMAND` | single approver |
| CRM activity | `schema.prisma:3084` | `is_approved`, `approved_by`, `approved_at` | `APPROVE_CRM_ACTIVITY` | single approver |
| Fund transfer | `schema.prisma:4055` | *none* | `APPROVE_FUND_TRANSFER` — **granted but never enforced**, see below | none |
| Payroll run | `schema.prisma:4928` | `status`, `approved_by`, `approved_at` | — | single approver |
| Stock take | `InventorySettings`, `schema.prisma:1683` | `discrepancy_approval_threshold` | — | a threshold, hard-coded in shape |
| Warranty claim | `schema.prisma:4087` | status enum incl. `APPROVED` | — | single approver |
...
2. **Never queue machine-generated documents behind a human by default.**
   `auto_approve_system_vouchers` (`schema.prisma:3393`) exists because holding
...
- **`Employee.manager_id`** — there is no reporting line on `Employee`
  (`schema.prisma:4436`). `hiring_manager_id` on `JobPost` and `manager_id` on
  `Project` are the only manager fields in the schema, and neither is a
  hierarchy.
- **`Department.head_employee_id`** — `Department` (`schema.prisma:4406`) is a
...
model ApprovalPolicy {
  ...
  /// `AccountingSettings.reports_approved_only` (schema.prisma:3396).
...
model ApprovalAction {
  ...
  /// (see the note at schema.prisma:1860) and a deleted user must not take the
  /// history with them.
  approver_id String?
...
|---|---|---|
| `PERMISSION` | everyone holding `approver_ref` on the entry's store | `UserStorePermission` (`schema.prisma:4016`) — the existing matrix |
...
means, and keeps its index (`schema.prisma:3495`). Every existing report, filter
...
already handles actors (`schema.prisma:1860`) — plus `approver_name` snapshotted
