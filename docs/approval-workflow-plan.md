...
2. **Never queue machine-generated documents behind a human by default.**
   `auto_approve_system_vouchers` (`schema.prisma:3523`) exists because holding
...
model ApprovalPolicy {
  ...
  /// `AccountingSettings.reports_approved_only` (schema.prisma:3526).
...
model ApprovalAction {
  ...
  /// (see the note at schema.prisma:1964) and a deleted user must not take the
  /// history with them.
  approver_id String?
...
means, and keeps its index (`schema.prisma:3625`). Every existing report, filter
...
already handles actors (`schema.prisma:1964`) — plus `approver_name` snapshotted
