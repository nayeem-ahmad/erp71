...
|---|---|---|---|---|
| Voucher | `schema.prisma:3446` | `approval_status`, `approved_by`, `approved_at`, `rejection_reason` | `APPROVE_VOUCHER` | tenant-wide on/off flag, anyone holding the permission |
| Leave request | `schema.prisma:5237` | `status`, `approvals_given`, `LeaveRequestApproval[]` (`:5276`) | `MANAGE_HR` | N levels from `LeaveType.approval_levels` (`:4525`), anyone holding the permission |
| Expense claim | `schema.prisma:4838` | `status`, `approved_by`, `approved_at`, `approver_note` | `MANAGE_HR` | single approver |
| Warehouse transfer | `schema.prisma:1703` | `requires_approval`, `approved_by`, `approval_date` | `APPROVE_GOODS_TRANSFER` | single approver |
| Product demand | `schema.prisma:1836` | `reviewed_by`, `reviewed_at`, per-line `quantity_approved` | `APPROVE_PRODUCT_DEMAND` | single approver |
| CRM activity | `schema.prisma:3076` | `is_approved`, `approved_by`, `approved_at` | `APPROVE_CRM_ACTIVITY` | single approver |
| Fund transfer | `schema.prisma:4047` | *none* | `APPROVE_FUND_TRANSFER` — **granted but never enforced**, see below | none |
| Payroll run | `schema.prisma:4920` | `status`, `approved_by`, `approved_at` | — | single approver |
| Stock take | `InventorySettings`, `schema.prisma:1683` | `discrepancy_approval_threshold` | — | a threshold, hard-coded in shape |
| Warranty claim | `schema.prisma:4079` | status enum incl. `APPROVED` | — | single approver |
...
2. **Never queue machine-generated documents behind a human by default.**
   `auto_approve_system_vouchers` (`schema.prisma:3385`) exists because holding
...
- **`Employee.manager_id`** — there is no reporting line on `Employee`
  (`schema.prisma:4428`). `hiring_manager_id` on `JobPost` and `manager_id` on
  `Project` are the only manager fields in the schema, and neither is a
  hierarchy.
- **`Department.head_employee_id`** — `Department` (`schema.prisma:4398`) is a
...
model ApprovalPolicy {
  ...
  /// `AccountingSettings.reports_approved_only` (schema.prisma:3388).
...
model ApprovalAction {
  ...
  /// (see the note at schema.prisma:1852) and a deleted user must not take the
  /// history with them.
  approver_id String?
...
|---|---|---|
| `PERMISSION` | everyone holding `approver_ref` on the entry's store | `UserStorePermission` (`schema.prisma:4008`) — the existing matrix |
| `TENANT_ROLE` | everyone with that tenant role | `TenantUser.role` (`schema.prisma:1207`) |
...
means, and keeps its index (`schema.prisma:3487`). Every existing report, filter
...
already handles actors (`schema.prisma:1852`) — plus `approver_name` snapshotted
