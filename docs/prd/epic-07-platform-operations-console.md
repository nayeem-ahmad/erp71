...
5. **Story 5: Platform-wide Tenant Feature Toggles**
   * **Description:** A platform admin can globally switch optional, platform-level features on or off for tenants — feedback widget, support widget, help widget, and voice features — without a deploy, alongside the existing key-value platform settings (SMS/email/payment-gateway credentials, maintenance mode).
   * Status: Done — `general.feedback_enabled` / `support_enabled` / `help_enabled` / `voice_enabled` keys in `apps/backend/src/platform-settings/platform-settings.service.ts`, `apps/backend/src/platform-settings/platform-settings.controller.ts`, "Tenant Features" tab at `apps/frontend/src/app/(app)/admin/platform-settings/tenant-features/page.tsx`. Note: these are global on/off switches, not per-tenant overrides — a true per-tenant `TenantFeature` model does not exist yet.

### Notes
All five stories were already implemented before this epic doc was written (Story 5 added 2026-07-05, after `feat(settings): fix settings password input focus loss and add platform admin tenant-features`). This file only closes the documentation gap — no functional changes accompany these entries. Story 4's partial status reflects a real product gap (issued keys can't yet authenticate anything) worth a follow-up, not a documentation-only note.
