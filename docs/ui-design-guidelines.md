...
- **Modals** — `ModalShell` is mandatory (migrate the 27 bypassers). Add `ModalHeader` (title `text-base font-semibold` + X) and `ModalFooter` subcomponents so header/footer stop diverging. Size guidance: `sm` confirm/simple create · `md`–`lg` standard forms · `xl`+ multi-column entry. Anything with tabs or >~10 fields should be a page, not a modal.
  - **Drawer variant** — `variant="drawer"` slides the same panel in from the inline edge at full height, for a surface read *beside* the list that opened it rather than on top of it (the activities list's per-lead timeline). Below `sm` it is the identical bottom sheet: a side drawer on a 360px phone is just a full-screen panel that arrived sideways. Reach for it when the list's filters and scroll position are the thing worth keeping; a plain `dialog` otherwise.
  - **Nesting** — a shell opened from inside another one is legitimate (the drawer's panel has its own edit and complete dialogs). `ModalShell` handles the two consequences itself: Escape closes only the innermost shell, and the inner backdrop is a light scrim rather than a second full dim. Nothing for the caller to do.
...
- **`FloatingPanel`** is the one sanctioned floating *surface*, and the time tracker is its only current use. What earns it the exception: it holds a form — choosing a task, a note, tags, manual logging — that is too big for a header and needs to be reachable from any page. **It is no longer what keeps a running clock visible**: that is `TimerChip` in the app header, which is on every `(app)` page, always in the same place, and covers nothing. The earlier version of this rule said no header could do that job; it was wrong, because the header lives in the `(app)` layout and survives client-side navigation exactly as the panel does. What the panel must obey — every one of these, or it is a FAB with extra steps:
  ...
  - it is **movable** (`useFloatingPanel` + `lib/floating-panel-position.ts`), so it can be put wherever it is least in the way. A panel opened from a header control passes that control as its `anchor` and opens just below it every time — the tracker drops out of `TimerChip`, and the header's other actions step aside while it is open, coming back when it closes (Escape closes it too). A panel with no anchor on screen remembers where it was left instead (one localStorage key per panel);
  - it can always be collapsed to its header, and — now that the header chip carries the running clock — dismissed outright even while a timer counts, because closing it no longer hides anything that is still happening;
