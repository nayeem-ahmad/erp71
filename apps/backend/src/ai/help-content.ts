/**
 * The assistant's product knowledge — how the app works, where features live,
 * and how to do things. This is what lets it answer "how do I…", "where is…"
 * and "what does X do" questions, which the data tools cannot: those report the
 * tenant's own numbers, this explains the product itself.
 *
 * SOURCE OF TRUTH: this mirrors the user-facing Help Center at
 * `apps/frontend/src/lib/localization/messages/en/help.ts` (the English FAQ).
 * The two live in different apps and cannot import each other, so they are kept
 * in sync by hand — when the Help Center FAQ changes, update this too. The
 * section ids here match that file's `sections` keys, and
 * `help-content.spec.ts` fails if the Help Center grows a section this file
 * does not cover, so a new documented feature cannot silently go unanswerable.
 *
 * Only the English copy is mirrored. The assistant replies in the user's
 * language from this English source, the same way it does for tool results.
 */
...
export interface HelpSection {
    /** Matches the `sections` key in the frontend Help Center. */
    id: string;
    title: string;
    faqs: HelpFaq[];
}
...
const STOPWORDS = new Set([
    ...
    'how', 'what', 'where', 'when', 'why', 'can', 'i', 'my', 'me', 'we', 'our', 'you', 'your',
    'it', 'this', 'that', 'with', 'from', 'at', 'as', 'be', 'by', 'up', 'get', 'set', 'use',
]);

/**
 * A few phrasings the FAQ answers under a different word. Kept deliberately tiny —
 * the corpus is small and matching is forgiving — so this only smooths the most
 * common vocabulary gaps between how a shopkeeper asks and how the FAQ is worded.
 */
const SYNONYMS: Record<string, string[]> = {
    staff: ['employee', 'team'],
    employee: ['staff'],
    invoice: ['sale', 'bill'],
    bill: ['invoice', 'purchase'],
    refund: ['return'],
    stock: ['inventory'],
    inventory: ['stock'],
    warehouse: ['stock'],
...
/**
 * Keyword search over the FAQ. The corpus is ~50 entries, so a scored token
 * overlap (question hits count for more than answer hits, plus a phrase bonus)
 * is enough — no index or embeddings. Returns the best few matches; an empty
 * result is a real answer ("no FAQ covers this"), not an error.
 */
export function searchHelp(query: string, limit = 4): HelpMatch[] {
    ...
    for (const section of HELP_SECTIONS) {
        ...
        for (const faq of section.faqs) {
            ...
            // A near-verbatim question is almost certainly the answer wanted.
            if (normalizedQuery.length > 4 && faq.q.toLowerCase().includes(normalizedQuery)) score += 5;
            if (score > 0) matches.push({ section: section.title, q: faq.q, a: faq.a, score });
        }
    }
...
/** Section titles, for when nothing matched and the model should offer areas. */
export function helpSectionTitles(): string[] {
    return HELP_SECTIONS.map((s) => s.title);
}
