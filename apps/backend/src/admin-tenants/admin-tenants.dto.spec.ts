...
/**
 * The feature DTO restates PLATFORM_FEATURE_KEYS by hand, and the admin UI
...
describe('UpdateAdminTenantFeaturesDto', () => {
    ...
    it('accepts every platform feature key', async () => {
        const payload = Object.fromEntries(PLATFORM_FEATURE_KEYS.map((key) => [key, true]));
    ...
    it('accepts null for every key, which is how an override is cleared', async () => {
        const payload = Object.fromEntries(PLATFORM_FEATURE_KEYS.map((key) => [key, null]));
