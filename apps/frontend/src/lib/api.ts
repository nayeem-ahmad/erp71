...
export const api = {
    ...
    // CRM Leads
    getLeads: (params?: { status?: string; source?: string; category?: string; priority?: string; assignedTo?: string; staleDays?: number; myActionsToday?: boolean; search?: string; page?: number; limit?: number; sortBy?: string; sortDir?: string; createdFrom?: string; createdTo?: string }) => {
        const query = new URLSearchParams();
        if (params?.status) query.set('status', params.status);
    ...
    // CRM Campaigns
    getCrmCampaigns: (params?: CreatedRangeParams) => fetchAllPages(withCreatedRange('/crm/campaigns', params)),
    ...
    // Loans
    getLoans: (params?: { direction?: string; status?: string; storeId?: string; search?: string; createdFrom?: string; createdTo?: string }) => {
        const q = new URLSearchParams();
        if (params?.direction) q.set('direction', params.direction);
        if (params?.status) q.set('status', params.status);
