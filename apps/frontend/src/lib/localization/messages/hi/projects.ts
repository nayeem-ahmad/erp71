export const projectsMessages = {
    title: "परियोजना प्रबंधन",
    subtitle: "काम, कार्य, समय और स्प्रिंट",
    newProject: "नई परियोजना",
    editProject: "परियोजना संपादित करें",
    empty: "अभी कोई परियोजना नहीं। काम पर नज़र रखने के लिए एक बनाएँ।",
    emptyFiltered: "इन फ़िल्टरों से कोई परियोजना मेल नहीं खाती।",
    searchPlaceholder: "कोड, नाम या ग्राहक खोजें",
    saved: "परियोजना सहेजी गई",
    saveFailed: "परियोजना सहेजी नहीं जा सकी",
    loadFailed: "परियोजना लोड नहीं हो सकी",
    deleted: "परियोजना हटा दी गई",
    deleteFailed: "परियोजना हटाई नहीं जा सकी",
    deleteProject: "परियोजना हटाएँ",
    deletePrompt: "{name} हटाएँ?\n\nइसके कार्य और उन पर दर्ज घंटे बने रहते हैं — केवल परियोजना आपकी सूचियों से हटती है।",

    fields: {
        code: "कोड",
        codeHint: "टास्क कुंजियाँ इसी से बनती हैं — {code}-14। नाम से सुझाया गया; चाहें तो बदलें।",
        codeEditHint: "बदलने पर पुरानी टास्क कुंजियाँ काम करती रहेंगी — वे नए कोड पर पहुँचेंगी।",
        name: "नाम",
        shortName: "संक्षिप्त नाम",
        shortNameHint: "बोर्ड कार्डों पर दिखता है जहाँ पूरा नाम नहीं समाता।",
        description: "विवरण",
        customer: "ग्राहक",
        type: "प्रकार",
        status: "स्थिति",
        priority: "प्राथमिकता",
        visibility: "दृश्यता",
        manager: "प्रबंधक",
        branch: "शाखा",
        startDate: "आरंभ तिथि",
        targetEndDate: "लक्षित समाप्ति",
        actualEndDate: "वास्तविक समाप्ति",
        budget: "बजट",
        tasks: "कार्य",
        progress: "प्रगति",
        actions: "कार्रवाइयाँ",
        project: "परियोजना",
        assignee: "सौंपा गया",
        sprint: "स्प्रिंट",
        dueDate: "नियत",
    },

    status: {
        DRAFT: "ड्राफ़्ट",
        ACTIVE: "सक्रिय",
        ON_HOLD: "रोका गया",
        COMPLETED: "पूर्ण",
        CANCELLED: "रद्द",
    },

    visibility: {
        PUBLIC: "सार्वजनिक",
        PRIVATE: "निजी",
    },

    visibilityHelp: {
        PUBLIC: "कार्यक्षेत्र में परियोजनाएँ देख सकने वाले सभी लोग।",
        PRIVATE: "केवल परियोजना टीम में जोड़े गए लोग।",
    },

    priority: {
        LOW: "निम्न",
        MEDIUM: "मध्यम",
        HIGH: "उच्च",
        URGENT: "अत्यावश्यक",
    },

    validation: {
        nameRequired: "परियोजना को एक नाम दें।",
        codeInvalid: "2–12 अक्षर: बड़े अक्षर, अंक और हाइफ़न, शुरुआत एक अक्षर से।",
    },

    statusCategory: {
        TODO: "करना है",
        IN_PROGRESS: "चल रहा है",
        DONE: "हो गया",
    },

    tasks: {
        title: "कार्य",
        subtitle: "हर परियोजना के सभी कार्य",
        empty: "इन फ़िल्टरों से कोई कार्य मेल नहीं खाता।",
        searchPlaceholder: "कार्य शीर्षक खोजें",
        mine: "मुझे सौंपे गए",
        anyone: "कोई भी",
        allProjects: "सभी परियोजनाएँ",
        anyStatus: "कोई भी स्थिति",
        anyPriority: "कोई भी प्राथमिकता",
        bulkDeletePrompt: "{count, plural, one {# चयनित कार्य} other {# चयनित कार्य}} हटाएँ?\n\nये हर सूची, बोर्ड और स्प्रिंट से हट जाएँगे। इसे वापस नहीं लिया जा सकता।",
        deletedCount: "{count, plural, one {# कार्य हटाया गया} other {# कार्य हटाए गए}}",
        deleteFailedCount: "{count, plural, one {# कार्य नहीं हटाया जा सका} other {# कार्य नहीं हटाए जा सके}}",
    },

    tabs: {
        overview: "अवलोकन",
        tasks: "कार्य",
        board: "बोर्ड",
        backlog: "बैकलॉग",
        team: "टीम",
        milestones: "मील के पत्थर",
        time: "समय",
    },

    overview: {
        estimated: "अनुमानित",
        logged: "दर्ज",
        remaining: "शेष",
        complete: "पूर्ण",
        noMilestones: "अभी कोई मील का पत्थर नहीं।",
        noTeam: "इस परियोजना पर अभी कोई नहीं है।",
    },

    team: {
        accessNote: "यह सूची तय करती है कि परियोजना कौन खोल सकता है। किसी को हटाने पर उसकी पहुँच भी चली जाती है।",
        add: "सदस्य जोड़ें",
        role: "भूमिका",
        MANAGER: "प्रबंधक",
        MEMBER: "सदस्य",
        VIEWER: "दर्शक",
        remove: "परियोजना से हटाएँ",
        added: "परियोजना में जोड़ा गया",
        addFailed: "वह व्यक्ति जोड़ा नहीं जा सका",
        removed: "परियोजना से हटाया गया",
        removeFailed: "वह व्यक्ति हटाया नहीं जा सका",
        person: "व्यक्ति",
        noLogin: "लॉगिन नहीं",
        noLoginHint: "बिना वर्कस्पेस खाते वाले कर्मचारी टीम में हो सकते हैं और कार्य रख सकते हैं, पर उन्हें देखने के लिए साइन इन नहीं कर सकते।",
    },

    milestones: {
        add: "मील का पत्थर जोड़ें",
        name: "मील के पत्थर का नाम",
        targetDate: "लक्षित तिथि",
        markComplete: "पूर्ण चिह्नित करें",
        reopen: "फिर खोलें",
    },


    /**
     * User stories. The requirement in the words of whoever wants it, with the
     * tasks that deliver it filed under it — see `ProjectStoriesCard`.
     */
    stories: {
        title: 'यूज़र स्टोरी',
        add: "नई यूज़र स्टोरी",
        edit: 'स्टोरी संपादित करें',
        empty: 'अभी कोई यूज़र स्टोरी नहीं है। लिखें कि इस प्रोजेक्ट से क्या माँगा जा रहा है।',
        code: "स्टोरी आईडी",
        codeHint: "स्वतः नंबर देने के लिए खाली छोड़ें, जैसे {example}।",
        codeAuto: "स्वचालित",
        codeRequired: "स्टोरी को एक आईडी दें।",
        codeNoSpaces: "स्टोरी आईडी में स्पेस नहीं हो सकते।",
        projectRequired: "एक परियोजना चुनें।",
        pickProject: "परियोजना चुनें",
        asA: 'एक',
        iWant: 'मैं चाहता/चाहती हूँ',
        soThat: 'ताकि',
        asAPlaceholder: 'दुकानदार',
        iWantPlaceholder: 'काउंटर पर bKash से भुगतान लेना',
        soThatPlaceholder: 'मुझे खुले पैसे न गिनने पड़ें',
        narrative: 'एक {asA} के रूप में, मैं {iWant} चाहता/चाहती हूँ ताकि {soThat}।',
        acceptance: 'स्वीकृति की शर्तें',
        acceptancePlaceholder: 'इस स्टोरी को पूरा मानने से पहले क्या सही होना चाहिए।',
        points: 'पॉइंट',
        pointsHint: 'सापेक्ष आकार, घंटे नहीं — घंटे टास्क पर ही रहते हैं।',
        pointsShort: '{points} पॉइंट',
        statuses: {
            BACKLOG: 'बैकलॉग',
            READY: 'तैयार',
            IN_PROGRESS: 'चालू',
            DONE: 'पूर्ण',
        },
        taskProgress: '{done}/{total} टास्क',
        noTasks: 'इस स्टोरी के अंतर्गत अभी कोई टास्क नहीं है।',
        addTask: 'टास्क जोड़ें',
        taskPlaceholder: 'क्या बनाना है',
        none: 'कोई स्टोरी नहीं',
        field: 'यूज़र स्टोरी',
        created: 'स्टोरी बनाई गई',
        updated: 'स्टोरी सहेजी गई',
        deleted: 'स्टोरी हटाई गई',
        deleteStory: 'स्टोरी हटाएँ',
        deletePrompt: '{title} हटाएँ?\n\nइसके टास्क बने रहेंगे — बस समूह हट जाएगा।',
        saveFailed: 'स्टोरी सहेजी नहीं जा सकी',
        deleteFailed: 'स्टोरी हटाई नहीं जा सकी',
        loadFailed: 'यूज़र स्टोरी लोड नहीं हो सकीं',
        titleRequired: 'स्टोरी को एक शीर्षक दें।',
        statusDerived: "कोई टास्क शुरू होते ही स्थिति टास्क के अनुसार चलती है।",
    },

    /**
     * The cross-project backlog at /projects/stories. Its own block rather than
     * more keys on `stories`, which belongs to the card on the project page —
     * the same split `tasks` already makes against the per-project task table.
     */
    storyList: {
        title: "यूज़र स्टोरी",
        subtitle: "हर परियोजना की सभी स्टोरी",
        empty: "अभी कोई यूज़र स्टोरी नहीं है। एक लिखें, या स्प्रेडशीट से बैकलॉग इम्पोर्ट करें।",
        emptyFiltered: "इन फ़िल्टरों से कोई स्टोरी मेल नहीं खाती।",
        searchPlaceholder: "स्टोरी शीर्षक या उसकी माँग खोजें",
        allProjects: "सभी परियोजनाएँ",
        anyStatus: "कोई भी स्थिति",
        anyPriority: "कोई भी प्राथमिकता",
        story: "स्टोरी",
        openInProject: "इसकी परियोजना में खोलें",
        anyEpic: "कोई भी एपिक",
        noEpic: "कोई एपिक नहीं",
    },

    /**
     * Epics — the parent of user stories. `epics` is the card and form on the
     * project page; `epicList` is the cross-project list at /projects/epics.
     */
    epics: {
        title: "एपिक",
        add: "नया एपिक",
        edit: "एपिक संपादित करें",
        empty: "अभी कोई एपिक नहीं। बड़े दायरे को ट्रैक करने के लिए संबंधित यूज़र स्टोरी को एक एपिक में रखें।",
        code: "एपिक आईडी",
        codeHint: "अपने आप क्रमांक के लिए खाली छोड़ें, जैसे {example}।",
        codeAuto: "स्वचालित",
        codeRequired: "एपिक को एक आईडी दें।",
        codeNoSpaces: "एपिक आईडी में खाली जगह नहीं हो सकती।",
        projectRequired: "एक प्रोजेक्ट चुनें।",
        pickProject: "प्रोजेक्ट चुनें",
        description: "विवरण",
        descriptionPlaceholder: "यह काम क्या देगा, और यह क्यों ज़रूरी है।",
        color: "रंग",
        startDate: "आरंभ तिथि",
        targetDate: "लक्ष्य तिथि",
        statuses: {
            OPEN: "खुला",
            IN_PROGRESS: "प्रगति में",
            DONE: "पूर्ण",
            CANCELLED: "रद्द",
        },
        storyProgress: "{done}/{total} स्टोरी",
        pointsProgress: "{done}/{total} अंक",
        taskProgress: "{done}/{total} टास्क",
        noStories: "इस एपिक में अभी कोई स्टोरी नहीं।",
        addStory: "स्टोरी जोड़ें",
        storyPlaceholder: "इस एपिक की एक स्टोरी का शीर्षक",
        field: "एपिक",
        none: "कोई एपिक नहीं",
        created: "एपिक बनाया गया",
        updated: "एपिक सहेजा गया",
        deleted: "एपिक हटाया गया",
        deleteEpic: "एपिक हटाएँ",
        deletePrompt: "{title} हटाएँ?\n\nइसकी स्टोरी बनी रहेंगी — बस उनका समूह हट जाएगा।",
        saveFailed: "एपिक सहेजा नहीं जा सका",
        deleteFailed: "एपिक हटाया नहीं जा सका",
        loadFailed: "एपिक लोड नहीं हो सके",
        titleRequired: "एपिक को एक शीर्षक दें।",
    },

    /** The per-project tree at /projects/[id]/backlog. */
    backlog: {
        title: "बैकलॉग",
        subtitle: "एपिक, यूज़र स्टोरी और उनके टास्क एक ट्री में",
        open: "बैकलॉग",
        searchPlaceholder: "आईडी, शीर्षक या असाइनी खोजें",
        hideDone: "पूरे हुए छिपाएँ",
        expand: "फैलाएँ",
        expandEpics: "केवल एपिक",
        expandStories: "एपिक और स्टोरी",
        expandAll: "सब कुछ",
        noEpic: "कोई एपिक नहीं",
        unplanned: "बिना स्टोरी के टास्क",
        empty: "इस बैकलॉग में अभी कुछ नहीं है। किसी एपिक से शुरू करें, या सीधे एक स्टोरी लिखें।",
        emptyFiltered: "इन फ़िल्टर से कुछ मेल नहीं खाता।",
        addStory: "स्टोरी जोड़ें",
        addTask: "टास्क जोड़ें",
        subtasks: "{count} सबटास्क",
        hours: "{logged}/{estimate} घं",
        unassigned: "असाइन नहीं",
        expandRow: "फैलाएँ",
        collapseRow: "समेटें",
        loadFailed: "बैकलॉग लोड नहीं हो सका",
        createFailed: "जोड़ा नहीं जा सका",
    },

    epicList: {
        title: "एपिक",
        subtitle: "हर प्रोजेक्ट के सभी एपिक",
        empty: "अभी कोई एपिक नहीं। एक लिखें, या स्प्रेडशीट से इम्पोर्ट करें।",
        emptyFiltered: "इन फ़िल्टर से कोई एपिक मेल नहीं खाता।",
        searchPlaceholder: "एपिक आईडी या शीर्षक खोजें",
        allProjects: "सभी प्रोजेक्ट",
        anyStatus: "कोई भी स्थिति",
        anyPriority: "कोई भी प्राथमिकता",
        epic: "एपिक",
        progress: "प्रगति",
        stories: "स्टोरी",
        openInProject: "इसके प्रोजेक्ट में खोलें",
    },

    task: {
        title: "कार्य",
        newTask: "नया कार्य",
        editTask: "कार्य संपादित करें",
        titleField: "शीर्षक",
        assignee: "सौंपा गया",
        dueDate: "नियत तिथि",
        milestone: "मील का पत्थर",
        estimate: "अनुमान (घं.)",
        subtasks: "उप-कार्य",
        noTasks: "अभी कोई कार्य नहीं।",
        deleted: "कार्य हटा दिया गया",
        deleteTask: "कार्य हटाएँ",
        deletePrompt: "{title} हटाएँ?\n\nयह हर सूची, बोर्ड और स्प्रिंट से हट जाएगा। इसे वापस नहीं लिया जा सकता।",
        deleteFailed: "कार्य हटाया नहीं जा सका",
        created: "कार्य बनाया गया",
        updated: "कार्य अपडेट हुआ",
        editTitle: "शीर्षक संपादित करें",
        renameFailed: "कार्य का नाम नहीं बदला जा सका",
        details: "विवरण",
        unassigned: "अनासौंपा",
        saveFailed: "कार्य अपडेट नहीं हो सका",
        selectProject: "एक परियोजना चुनें",
        projectRequired: "वह परियोजना चुनें जिससे यह कार्य जुड़ा है।",
        titleRequired: "कार्य को एक शीर्षक दें।",
        noProjects: "पहले एक परियोजना बनाएँ — हर कार्य किसी एक से जुड़ा होता है।",
        createFailed: "कार्य नहीं बन सका",
        openFull: "पूरा पृष्ठ खोलें",
        copyLink: "लिंक कॉपी करें",
        linkCopied: "लिंक कॉपी हो गया",
        copyLinkFailed: "लिंक कॉपी नहीं हो सका",
        assigneeNoTeam: "इस प्रोजेक्ट की टीम में अभी कोई नहीं है।",
        assigneeAddTeam: "टीम सदस्य जोड़ें",
        assigneeLoadFailed: "इस प्रोजेक्ट की टीम नहीं पढ़ी जा सकी।",
        copyKey: "कार्य कुंजी कॉपी करें",
        keyCopied: "कार्य कुंजी कॉपी हो गई",
        copyKeyFailed: "कार्य कुंजी कॉपी नहीं हो सकी",
        moreActions: "और विकल्प",
        openProject: "प्रोजेक्ट खोलें",
        createdBy: "{date} को {name} ने बनाया",
        createdOn: "{date} को बनाया गया",
        updatedAt: "अंतिम अपडेट {when}",
        unavailableTitle: "यह कार्य उपलब्ध नहीं है",
        unavailableBody: "हो सकता है इसे हटा दिया गया हो, या यह किसी ऐसे निजी प्रोजेक्ट का हिस्सा हो जिसके आप सदस्य नहीं हैं।",
        loadFailed: "यह कार्य लोड नहीं हो सका।",
        goToTasks: "कार्यों पर जाएँ",
    },

    description: {
        title: "विवरण",
        add: "विवरण जोड़ें…",
        edit: "विवरण संपादित करें",
        empty: "अभी कोई विवरण नहीं।",
        placeholder: "जो कुछ भी यह काम करने वाले को जानना चाहिए।",
        saveFailed: "विवरण सहेजा नहीं जा सका",
        addHint: "क्या चाहिए, और कैसे पता चलेगा कि काम पूरा हुआ। मार्कडाउन और पेस्ट किए स्क्रीनशॉट दोनों चलते हैं।",
    },

    dates: {
        title: "तिथियाँ",
        start: "आरंभ तिथि",
        due: "नियत तिथि",
        inverted: "आरंभ नियत तिथि के बाद है।",
        saveFailed: "तिथियाँ सहेजी नहीं जा सकीं",
        none: "कोई नियत तिथि नहीं",
        clear: "नियत तिथि हटाएँ",
    },

    activity: {
        title: "गतिविधि",
        comment: "टिप्पणी",
        commentPlaceholder: "टिप्पणी जोड़ें…",
        editComment: "टिप्पणी संपादित करें",
        empty: "यहाँ अभी कुछ नहीं हुआ।",
        loadFailed: "गतिविधि लोड नहीं हो सकी।",
        saveFailed: "वह सहेजा नहीं जा सका",
        someone: "कोई",
        watch: "देखें",
        watching: "देखा जा रहा है",
        watchHint: "यह कार्य आगे बढ़ने, दोबारा सौंपे जाने या टिप्पणी मिलने पर देखने वालों को सूचित किया जाता है।",
        none: '—',
        types: {
            CREATED: "ने यह कार्य बनाया",
            RENAMED: "ने इसका नाम “{from}” से “{to}” किया",
            STATUS_CHANGED: "ने इसे {from} से {to} में ले जाया",
            ASSIGNED: "ने इसे {to} को सौंपा",
            UNASSIGNED: "ने सौंपा गया व्यक्ति हटाया",
            PRIORITY_CHANGED: "ने प्राथमिकता {from} से {to} की",
            DATES_CHANGED: "ने तिथियाँ बदलीं",
            LABELS_CHANGED: "ने लेबल बदले (अब {count})",
            RE_ESTIMATED: "ने {from} घं. से {to} घं. पुनःअनुमान लगाया",
        },
    },

    timeTags: {

        title: "घंटा-लॉग टैग",

        hint: "ये *घंटे* को वर्गीकृत करते हैं, कार्य को नहीं — \"बिल योग्य\", \"सपोर्ट\", \"दोबारा काम\"। लेबल से अलग सूची, और घंटा-लॉग रिपोर्ट इसी से जोड़ सकती है।",

        empty: "अभी कोई टैग नहीं।",

        add: "टैग जोड़ें",

        namePlaceholder: "जैसे बिल योग्य",

        untagged: "{count} घंटा-लॉग से हटाया गया।",

    },

    labels: {
        title: "लेबल",
        hint: "हर परियोजना में साझा, ताकि वर्कस्पेस-व्यापी कार्य सूची और स्प्रिंट बोर्ड इन पर फ़िल्टर कर सकें।",
        empty: "अभी कोई लेबल नहीं।",
        add: "लेबल जोड़ें",
        namePlaceholder: "जैसे अवरुद्ध",
        color: "रंग",
        saveFailed: "लेबल अपडेट नहीं हो सके",
        untagged: "{count} कार्य से हटाया गया।",
        coverNote: "पहला लेबल कार्ड का रंग तय करता है।",
        manage: "लेबल प्रबंधित करें",
        colors: {
            GRAY: "धूसर",
            BLUE: "नीला",
            EMERALD: "हरा",
            AMBER: "अंबर",
            RED: "लाल",
            PURPLE: "बैंगनी",
        },
    },

    columns: {
        title: "बोर्ड कॉलम",
        subtitle: "इस परियोजना के बोर्ड के कॉलम, और हर एक में कितने कार्ड रह सकते हैं।",
        add: "कॉलम जोड़ें",
        empty: "इस बोर्ड में अभी कोई कॉलम नहीं।",
        loadFailed: "कॉलम लोड नहीं हो सके।",
        wip: 'WIP',
        noLimit: "कोई नहीं",
        overLimit: "{name} अपनी सीमा से अधिक है",
        wipHint: "WIP सीमा सलाहकारी है — सीमा पार होने पर कॉलम चिह्नित होता है, रोका नहीं जाता। परियोजना सेटअप में टेनेंट डिफ़ॉल्ट बदलने से नई परियोजनाओं की शुरुआत बदलती है, यह बोर्ड नहीं।",
    },

    attachments: {
        title: "संलग्नक",
        add: "फ़ाइल संलग्न करें",
        empty: "अभी कुछ संलग्न नहीं।",
        loadFailed: "संलग्नक लोड नहीं हो सके।",
        uploadFailed: "वह फ़ाइल संलग्न नहीं हो सकी",
        deleteFile: "संलग्नक हटाएँ",
        preview: "पूर्वावलोकन",
        hint: "JPEG, PNG, WebP या PDF, 5 MB तक।",
        tooLarge: "वह फ़ाइल 5 MB से बड़ी है।",
        unsupported: "JPEG, PNG, WebP या PDF उपयोग करें।",
    },

    cover: {
        title: "कवर",
        none: "कोई कवर नहीं",
        pick: "कवर रंग {color}",
    },

    /** The task card's own furniture: its bottom tabs and its sidebar chart. */
    card: {
        tabs: {
            comments: "टिप्पणियाँ",
            activity: "गतिविधि",
            time: "घंटा लॉग",
            remaining: "शेष",
            attachments: "अनुलग्नक",
        },
        record: "कार्य अभिलेख",
        addChecklistItem: "आइटम जोड़ें",
        logTime: "समय दर्ज करें",
        timeTitle: "घंटे",
        spark: {
            title: "शेष",
            hover: "विवरण के लिए होवर करें",
        },
    },

    checklist: {
        title: "जाँच-सूची",
        add: "आइटम जोड़ें",
        placeholder: "क्या करना है?",
        empty: "अभी कोई जाँच-सूची आइटम नहीं।",
        progress: "{total} में से {done}",
        allDone: "सब हो गया",
        moveUp: "ऊपर ले जाएँ",
        moveDown: "नीचे ले जाएँ",
        deleteItem: "आइटम हटाएँ",
        saveFailed: "जाँच-सूची अपडेट नहीं हो सकी",
    },

    quickAdd: {
        placeholder: "कार्य जोड़ें — शीर्षक, फिर Enter",
        hint: "उसी पंक्ति में विवरण: @व्यक्ति  #लेबल  !high  ~3h  >शुक्रवार",
        add: "जोड़ें",
        more: "और फ़ील्ड",
        today: "आज",
        tomorrow: "कल",
        me: "मैं",
        setPriority: "प्राथमिकता तय करें",
        addDescription: "+ विवरण जोड़ें",
        saveAndAdd: "सहेजें और दूसरा जोड़ें",
        suggestions: "सुझाव",
    },

    timer: {
        running: "चल रहा है",
        open: "समय ट्रैकर",
        openTracker: "ट्रैकर खोलें",
        noTask: "कोई कार्य नहीं",
        start: "शुरू करें",
        stop: "रोकें",
        elsewhere: "किसी अन्य कार्य पर टाइमर पहले से चल रहा है",
        idleHint: "कुछ नहीं चल रहा। टाइमर शुरू करने या समय लॉग करने के लिए क्लिक करें।",
        startedAt: "शुरू हुआ",
        clickToOpen: "ट्रैकर खोलने के लिए क्लिक करें",
    },

    remaining: {
        chart: "समय के साथ शेष घंटे",
        chartNow: "अभी",
        chartUpNote: "बढ़े — काम जोड़ा गया",
        history: "शेष घंटों का इतिहास",
        notePlaceholder: "जैसे ग्राहक ने दो और कमरे जोड़े",
        empty: "अभी कोई बदलाव दर्ज नहीं।",
        by: "द्वारा",
        sources: {
            TASK_CREATED: "कार्य बनाया गया",
            TIME_LOGGED: "समय दर्ज हुआ",
            RE_ESTIMATED: "पुनःअनुमानित",
            TASK_COMPLETED: "कार्य पूर्ण",
            TASK_REOPENED: "कार्य फिर खुला",
            TIME_ENTRY_DELETED: "समय प्रविष्टि हटाई गई",
        },
        hint: "शेष घंटे बचे हुए काम का आपका अपना अनुमान हैं — अनुमान में से दर्ज घंटे घटाकर नहीं। जब काम अपेक्षा से बड़ा निकले तो इसे बदलें।",
    },

    time: {
        log: "समय दर्ज करें",
        hours: "घंटे",
        workDate: "तिथि",
        note: "नोट",
        remainingAfter: "इसके बाद शेष",
        remainingHint: "इन घंटों के बाद जो बचता है वह डिफ़ॉल्ट है। काम बढ़ गया हो तो बदलें।",
        logHint: "घंटे दर्ज करें, शेष संशोधित करें, या दोनों — एक फ़ॉर्म, एक सहेजना।",
        empty: "अभी कोई समय दर्ज नहीं।",
        logged: "समय दर्ज हुआ",
        deleted: "समय प्रविष्टि हटाई गई",
        totalHours: "कुल घंटे",
        deleteEntryTitle: "यह समय प्रविष्टि हटाएँ?",
        deleteEntryPrompt: "{date} के {hours} घंटे दर्ज कुल समय से घट जाएँगे।",
    },

    hourLogs: {
        title: "घंटा लॉग",
        subtitle: "किसी कार्य पर दर्ज हर घंटा, एक ही सूची में",
        logHours: "घंटे दर्ज करें",
        openReport: "रिपोर्ट",
        searchPlaceholder: "कार्य शीर्षक या नोट खोजें",
        allProjects: "सभी परियोजनाएँ",
        allPeople: "सभी",
        mine: "मेरे घंटे",
        anyTask: "कोई भी कार्य",
        from: "से",
        to: "तक",
        person: "व्यक्ति",
        unattributed: "अनिर्दिष्ट",
        empty: "इस सीमा में कोई घंटा दर्ज नहीं।",
        loadFailed: "घंटा लॉग लोड नहीं हो सके।",
        selectProjectFirst: "कार्य चुनने के लिए पहले परियोजना चुनें।",
        selectTask: "एक कार्य चुनें",
        searchTasks: "कार्य खोजें…",
        loadingTasks: "कार्य लोड हो रहे हैं…",
        trackerNoTasks: "आपको कोई कार्य नहीं सौंपा गया है।",
        trackerNoMatches: "इस खोज से कोई कार्य मेल नहीं खाता।",
        clearTask: "चयनित कार्य हटाएँ",
        scope: "किसके कार्य दिखाएँ",
        scopeMine: "मेरे",
        scopeEveryone: "सभी",
        noTasks: "इस परियोजना में अभी कोई कार्य नहीं।",
        logFailed: "वे घंटे दर्ज नहीं हो सके।",
        editEntry: "प्रविष्टि संपादित करें",
        updated: "प्रविष्टि अपडेट हुई",
        updateFailed: "वह प्रविष्टि अपडेट नहीं हो सकी।",
        deleteEntry: "प्रविष्टि हटाएँ",
        deletePrompt: "{date} को दर्ज {hours} घं. हटाएँ?\n\nघंटे शेष काम के रूप में कार्य पर वापस चले जाते हैं।",
        deleteFailed: "वह प्रविष्टि हटाई नहीं जा सकी।",
        rangeInvalid: "आरंभ तिथि समाप्ति तिथि पर या उससे पहले होनी चाहिए।",
        preset7: "पिछले 7 दिन",
        preset30: "पिछले 30 दिन",
        presetMonth: "इस महीने",
        presetCustom: "कस्टम सीमा",

        // Added with the day-grouped rework: the capture bar, the running
        // clock, spans, tags and the folded rows.
        capturePlaceholder: 'आप किस पर काम कर रहे हैं?',
        captureProject: 'किस प्रोजेक्ट पर दर्ज करें',
        captureTask: 'किस कार्य पर दर्ज करें',
        start: 'शुरू करें',
        stop: 'रोकें',
        running: 'टाइमर चल रहा है',
        discardTimer: 'यह टाइमर हटाएँ',
        timerMode: 'टाइमर से समय लें',
        manualMode: 'घंटे हाथ से भरें',
        startTime: 'शुरू का समय',
        endTime: 'खत्म का समय',
        spanHint: 'वैकल्पिक। दोनों दें या कोई नहीं। शुरू से पहले का अंत मतलब आधी रात पार हो गई।',
        hoursFromSpan: 'शुरू और खत्म के समय से निकाले गए।',
        tags: 'टैग',
        allTags: 'सभी टैग',
        noTags: 'अभी कोई टैग नहीं। प्रोजेक्ट सेटिंग्स में जोड़ें।',
        today: 'आज',
        yesterday: 'कल',
        total: 'कुल',
        duration: 'अवधि',
        partialDay: '{total} में से {shown} दिख रहे हैं ·',
        addDescription: 'विवरण जोड़ें',
        logAgain: 'इसके लिए फिर से टाइमर चलाएँ',
        expandGroup: 'इस पंक्ति की प्रविष्टियाँ दिखाएँ',
        collapseGroup: 'ये प्रविष्टियाँ छिपाएँ',
        pageOf: 'पृष्ठ {page} / {pages}',
        previous: 'पिछला',
        next: 'अगला',
        timerStarted: 'टाइमर शुरू हुआ',
        timerStartFailed: 'टाइमर शुरू नहीं हो सका।',
        timerStopFailed: 'टाइमर रोका नहीं जा सका।',
        timerUpdateFailed: 'टाइमर अपडेट नहीं हो सका।',
        timerDiscarded: 'टाइमर हटा दिया गया। कुछ भी दर्ज नहीं हुआ।',
        timerStartedAt: 'शुरू हुआ',
        tracker: 'टाइम ट्रैकर',
        trackerMove: 'टाइम ट्रैकर हटाएँ-सरकाएँ',
        trackerCollapse: 'टाइम ट्रैकर समेटें',
        trackerExpand: 'टाइम ट्रैकर खोलें',
        trackerClose: 'टाइम ट्रैकर छिपाएँ',
        timerAlreadyRunning: '"{task}" पर पहले से टाइमर चल रहा है। पहले उसे रोकें।',
        timerOverlapped: 'ये घंटे "{task}" पर पहले दर्ज समय से टकरा रहे हैं।',
        overlapTitle: 'ये घंटे आपस में टकरा रहे हैं',
        overlapPrompt: 'फिर भी दोनों प्रविष्टियाँ रखें?',
        overlapKeepBoth: 'दोनों रखें',

        view: 'दृश्य',
        viewDay: 'दिनवार दृश्य',
        viewList: 'सूची दृश्य',
    },

    hourLogReport: {
        title: "घंटा लॉग रिपोर्ट",
        subtitle: "तिथि सीमा में दर्ज घंटे, सारांशित",
        groupBy: "इसके अनुसार सारांश",
        openLogs: "घंटा लॉग खोलें",
        group: {
            task: "कार्य",
            date: "तिथि",
            week: "सप्ताह",
            month: "महीना",
            user: "व्यक्ति",
            project: "परियोजना",
            tag: "टैग",
        },
        totalHours: "कुल घंटे",
        entries: "प्रविष्टियाँ",
        days: "काम के दिन",
        people: "लोग",
        tasks: "कार्य",
        projects: "परियोजनाएँ",
        avgPerDay: "प्रति दिन औसत",
        hours: "घंटे",
        share: "हिस्सा",
        empty: "इस सीमा में कोई घंटा दर्ज नहीं हुआ।",
        loadFailed: "रिपोर्ट लोड नहीं हो सकी।",
        rangeHint: "कुल में सीमा के भीतर की तिथि वाला काम शामिल है, चाहे वह कभी भी दर्ज हुआ हो।",
        multiTagged: "{count} प्रविष्टियों पर एक से अधिक टैग हैं, इसलिए हिस्से मिलकर 100% से ज़्यादा होते हैं।",
    },

    board: {
        title: "बोर्ड",
        kanban: "कानबान",
        scrum: "स्क्रम",
        allTasks: "सभी कार्य",
        noSprint: "कोई स्प्रिंट नहीं चल रहा। स्क्रम मोड उपयोग करने के लिए स्प्रिंट से एक शुरू करें।",
        emptyColumn: "यहाँ कुछ नहीं",
        noMatches: "फ़िल्टरों से कुछ मेल नहीं खाता",
        laneUnassigned: "असाइन नहीं",
        laneNoStory: "कोई यूज़र स्टोरी नहीं",
        laneCards: "{count} कार्ड",
        laneDone: "{count} में से {done} पूरे",
        laneCollapse: "लेन समेटें",
        laneExpand: "लेन खोलें",
        laneOtherProject: "यह स्टोरी दूसरे प्रोजेक्ट की है, इसलिए यह कार्ड इसमें नहीं जुड़ सकता",
        laneColumn: "स्तंभ",
        moveFailed: "वह कार्ड ले जाया नहीं जा सका",
        createFailed: "कार्य नहीं बन सका",
        columnTotal: "घं. शेष",
        addCard: "कार्ड जोड़ें",
        newCardPlaceholder: "क्या करना है?",
        assignCardTo: "कार्ड इन्हें दें",
        filters: {
            assignee: "सौंपा गया",
            priority: "प्राथमिकता",
            due: "नियत",
            unassigned: "अनासौंपा",
            overdue: "मेयादोत्तीर्ण",
            dueToday: "आज नियत",
            dueThisWeek: "इस सप्ताह नियत",
            noDueDate: "कोई नियत तिथि नहीं",
            label: "लेबल",
            noLabel: "कोई लेबल नहीं",
            clear: "साफ़ करें",
            showing: "{total} कार्डों में से {shown}",
        },
        card: {
            drag: "ले जाने के लिए खींचें",
            open: "कार्य खोलें",
            project: "परियोजना",
            overdue: "मेयादोत्तीर्ण",
            dueToday: "आज नियत",
            due: "{date} को नियत",
            done: "हो गया",
            hasDescription: "विवरण है",
            checklist: "{total} में से {done} जाँच-सूची आइटम पूर्ण",
            comments: "{count} टिप्पणी",
            subtasks: "{count} उप-कार्य",
            unassigned: "अनासौंपा",
        },

        // The board's own appearance controls. The board is read very
        // differently by a lead scanning for what is late and by someone
        // working one lane, so the card layout is a setting, not a decision.
        view: {
            title: "रूप",
            scopeHint: "सिर्फ़ आपके लिए, इसी ब्राउज़र में — बाकी लोग बोर्ड अपने हिसाब से देखते हैं।",
            reset: "रीसेट",
            cardSize: "कार्ड का आकार",
            comfortable: "आरामदेह",
            compact: "संक्षिप्त",
            columnWidth: "स्तंभ की चौड़ाई",
            narrow: "सँकरा",
            standard: "मानक",
            wide: "चौड़ा",
            columnColor: "स्तंभ का रंग",
            tintNone: "सादा",
            tintCategory: "चरण अनुसार",
            scroll: "स्क्रॉल",
            scrollPage: "पूरा पेज",
            scrollColumn: "स्तंभों के भीतर",
            scrollHint: "स्तंभों के भीतर रखने पर बोर्ड का शीर्ष और हर स्तंभ का शीर्षक अपनी जगह बने रहते हैं। फ़ोन पर दोनों ही हाल में पेज स्क्रॉल होता है।",
            swimlanes: "स्विमलेन",
            swimlanesNone: "कोई नहीं",
            swimlanesAssignee: "असाइनी",
            swimlanesStory: "यूज़र स्टोरी",
            swimlanesHint: "हर स्तंभ को हर व्यक्ति या स्टोरी की एक पंक्ति में बाँटता है। कार्ड अपनी ही पंक्ति में स्तंभों के बीच चलते हैं।",
            motion: "गति",
            motionHint: "कार्ड और स्तंभ धीरे से अपनी जगह लेते हैं। डिवाइस कम गति माँगे तो यह लागू नहीं होता।",
            showOnCards: "कार्ड पर दिखाएँ",
            fields: {
                cover: "कवर",
                labels: "लेबल",
                project: "परियोजना",
                badges: "नियत तिथि और प्राथमिकता",
                details: "विवरण",
                assignee: "सौंपा गया",
            },
        },
    },

    sprint: {
        title: "स्प्रिंट",
        sprints: "स्प्रिंट",
        newSprint: "नया स्प्रिंट",
        name: "स्प्रिंट का नाम",
        goal: "लक्ष्य",
        startDate: "शुरू",
        endDate: "समाप्त",
        start: "स्प्रिंट शुरू करें",
        complete: "स्प्रिंट पूर्ण करें",
        planned: "नियोजित",
        active: "सक्रिय",
        completed: "पूर्ण",
        backlog: "बैकलॉग",
        addToSprint: "स्प्रिंट में जोड़ें",
        removeFromSprint: "बैकलॉग में लौटाएँ",
        committed: "प्रतिबद्ध",
        carriedOver: "{count} अधूरे कार्य बैकलॉग में लौटाए गए",
        empty: "अभी कोई स्प्रिंट नहीं।",
        emptyBacklog: "बैकलॉग खाली है।",
        oneActive: "पूरे वर्कस्पेस में एक बार में केवल एक स्प्रिंट चल सकता है।",
        subtitle: "हर परियोजना पर फैले समय-खंड",
        planning: "इस स्प्रिंट की योजना बनाएँ",
        loadFailed: "स्प्रिंट लोड नहीं हो सके",
        saveFailed: "स्प्रिंट सहेजा नहीं जा सका",
        created: "स्प्रिंट बनाया गया",
        started: "स्प्रिंट शुरू हुआ",
        completedMsg: "स्प्रिंट पूर्ण हुआ",
        deleted: "स्प्रिंट हटा दिया गया",
        deleteSprint: "स्प्रिंट हटाएँ",
        deletePrompt: "{name} हटाएँ?\n\nइसके कार्य बैकलॉग में लौट आते हैं — कुछ भी नहीं खोता।",
        emptySprint: "इस स्प्रिंट में अभी कोई कार्य नहीं।",
        noProjects: "अभी कोई परियोजना नहीं",
        tenantHint: "एक स्प्रिंट पूरे वर्कस्पेस पर फैला होता है। एक बार में केवल एक चल सकता है।",
        searchPlaceholder: "स्प्रिंट का नाम या लक्ष्य खोजें",
        emptyFiltered: "इन फ़िल्टरों से कोई स्प्रिंट मेल नहीं खाता।",
        anyStatus: "कोई भी स्थिति",
        allProjects: "सभी परियोजनाएँ",
        dates: "तिथियाँ",
        projects: "परियोजनाएँ",
        addWork: "काम जोड़ें",
        addWorkTitle: "{name} में जोड़ें",
        tabTasks: "टास्क",
        tabStories: "यूज़र स्टोरी",
        searchBacklog: "बैकलॉग में खोजें",
        emptyStories: "कोई खुली यूज़र स्टोरी नहीं।",
        openTasks: "{count} खुले टास्क",
        storyHint: "चुनी गई स्टोरी के वे सभी खुले टास्क जोड़ता है जो पहले से किसी स्प्रिंट में नहीं हैं।",
        selectedHours: "{count} चुने गए · {hours} घं",
        tasksAdded: "स्प्रिंट में {count} टास्क जोड़े गए",
        nothingAdded: "इन स्टोरी में स्प्रिंट के बाहर कोई खुला टास्क नहीं है।",
        removed: "बैकलॉग में लौटाया गया",
        colTask: "टास्क",
        colEstimate: "अनुमान (घं)",
        colSpent: "खर्च (घं)",
        colRemaining: "शेष (घं)",
        total: "कुल",
        stats: "मुख्य आँकड़े",
        statProgress: "प्रगति",
        statTasksDone: "पूरे टास्क",
        statDaysLeft: "शेष कार्यदिवस",
        statVariance: "आदर्श की तुलना में",
        ahead: "{hours} घं आगे",
        behind: "{hours} घं पीछे",
        onTrack: "सही राह पर",
        viewLabel: "व्यू",
        viewTable: "तालिका",
        viewCards: "कार्ड",
        searchTasks: "टास्क, स्टोरी, लोग, लेबल खोजें",
        noMatches: "खोज से कोई टास्क मेल नहीं खाता।",
        noColumnForProject: "{project} में \"{column}\" कॉलम नहीं है, इसलिए यह कार्ड वहाँ नहीं जा सकता।",
        laneLocked: "कार्ड अपनी ही पंक्ति में कॉलम बदलते हैं।",
        everyone: "सभी",
        info: "स्प्रिंट",
        noGoal: "कोई लक्ष्य तय नहीं",
        timeElapsed: "बीता समय",
        dayOf: "{total} में से दिन {day}",
        focusLane: "सिर्फ़ यह लेन दिखाएँ",
        expandAllLanes: "सभी लेन खोलें",
        lanesCollapsed: "{count} बंद",
    },

    burndown: {
        noTarget: "इस परियोजना की कोई लक्ष्य तिथि नहीं है, इसलिए तुलना के लिए कुछ नहीं — रेखा बताती है कि क्या हुआ।",
        title: "बर्नडाउन",
        ideal: "आदर्श",
        actual: "शेष",
        committed: "प्रतिबद्ध",
        hours: "घंटे",
        noData: "अभी कोई स्नैपशॉट नहीं। स्प्रिंट एक दिन चलने के बाद चार्ट भरता है।",
        rebuild: "इतिहास से पुनर्निर्माण करें",
        rebuilt: "{written} दिन पुनर्निर्मित, {skipped} रखे गए",
        weekendNote: "आदर्श रेखा शुक्र/शनि छोड़ती है।",
        tenantScope: "इस स्प्रिंट की सभी परियोजनाएँ",
        hoursUnit: "घं",
        tasksUnit: "टास्क",
        openTasks: "खुले टास्क",
        openTasksRightAxis: "खुले टास्क (दायाँ अक्ष)",
    },

    settings: {
        title: "परियोजना सेटअप",
        subtitle: "परियोजना प्रकार और बोर्ड कॉलम",
        types: "परियोजना प्रकार",
        typesHint: "आपके कामों की श्रेणियाँ — स्थापना, सर्विसिंग, फ़िट-आउट।",
        columns: "बोर्ड कॉलम",
        columnsHint: "नया प्रोजेक्ट जिन कॉलम से शुरू होता है। पहले से मौजूद प्रोजेक्ट अपनी सूची रखता है — उसे प्रोजेक्ट में जाकर बदलें। श्रेणी तय करती है कि क्या पूर्ण माना जाए।",
        category: "श्रेणी",
        categories: {
            TODO: "करना है",
            IN_PROGRESS: "चल रहा है",
            DONE: "हो गया",
        },
        isDefault: "नए कार्य यहाँ आते हैं",
        addType: "प्रकार जोड़ें",
        addColumn: "कॉलम जोड़ें",
        columnInUse: "पहले इस कॉलम से कार्य हटाएँ।",
        typeDeactivated: "वह प्रकार {count} परियोजनाओं में उपयोग हो रहा है, इसलिए हटाने के बजाय छिपा दिया गया।",
    },

    boards: {
        title: "बोर्ड",
        subtitle: "किसी भी परियोजना से चुने गए कार्ड",
        newBoard: "नया बोर्ड",
        name: "नाम",
        nameRequired: "बोर्ड को एक नाम दें।",
        description: "विवरण",
        empty: "अभी कोई बोर्ड नहीं। एक बनाएँ और किसी भी परियोजना से कार्य जोड़ें।",
        addTasks: "कार्य जोड़ें",
        addTasksTitle: "इस बोर्ड में कार्य जोड़ें",
        removeCard: "बोर्ड से हटाएँ",
        searchTasks: "कार्य खोजें",
        allProjects: "सभी परियोजनाएँ",
        selectedCount: "{count} चयनित",
        noResults: "कोई मेल खाता कार्य नहीं",
        loadAll: "सभी लोड करें",
        allOnBoard: "हर मेल खाता कार्य पहले से ही इस बोर्ड पर है।",
        loadMore: "और देखें",
        showingCount: "{total} में से {shown} दिख रहे हैं",
        added: "कार्य जोड़े गए",
        removed: "कार्ड हटाया गया",
        created: "बोर्ड बनाया गया",
        deleted: "बोर्ड हटा दिया गया",
        deleteBoard: "बोर्ड हटाएँ",
        deleteConfirm: "{name} हटाएँ? इस पर मौजूद कार्य प्रभावित नहीं होते।",
        boardSettings: "बोर्ड सेटिंग्स",
        backToBoard: "बोर्ड पर वापस जाएँ",
        unsorted: "अवर्गीकृत",
        unsortedHint: "ये कार्ड ऐसी स्थिति में हैं जिसे इस बोर्ड ने किसी कॉलम से नहीं जोड़ा।",
        unmappedDrop: "वह कॉलम {project} के लिए मैप नहीं है। बोर्ड सेटिंग्स में मैप करें।",
        columns: "कॉलम",
        columnName: "कॉलम का नाम",
        category: "श्रेणी",
        wipLimit: "WIP सीमा",
        wipInvalid: "WIP सीमा कम से कम 1 होनी चाहिए।",
        addColumn: "कॉलम जोड़ें",
        mappedStatuses: "मैप की गई स्थितियाँ",
        noMappings: "इस बोर्ड की किसी परियोजना के लिए मैप नहीं",
        createStatus: "बनाएँ",
        createStatusHint: "{project} में “{column}” स्थिति बनाएँ और इसे इस कॉलम से जोड़ें।",
        currentlyIn: "(अभी {column} में)",
        loadFailed: "कॉलम लोड नहीं हो सके।",
        deleteColumn: "कॉलम हटाएँ",
        deleteColumnConfirm: "\"{name}\" हटाएँ? इसके कार्ड अवर्गीकृत में चले जाते हैं, और इसकी स्थिति मैपिंग भी जाती है।",
        moveColumnUp: "ऊपर ले जाएँ",
        moveColumnDown: "नीचे ले जाएँ",

        searchCards: "कार्ड खोजें",
        dragColumn: "क्रम बदलने के लिए खींचें",
        columnActions: "कॉलम की क्रियाएँ",
        renameColumn: "नाम बदलें",
        moveColumnLeft: "बाएँ ले जाएँ",
        moveColumnRight: "दाएँ ले जाएँ",
        sortCards: "कार्ड क्रमबद्ध करें",
        sortBy: {
            due: "नियत तारीख से",
            priority: "प्राथमिकता से",
            title: "शीर्षक से",
            assignee: "ज़िम्मेदार व्यक्ति से",
        },
        moveAllCards: "सभी कार्ड यहाँ ले जाएँ",
        selectAllCards: "सभी कार्ड चुनें",
        noOtherColumn: "अभी कोई दूसरा कॉलम नहीं",
        selectCard: "कार्ड चुनें",
        moveSelectedTo: "चुने हुए यहाँ ले जाएँ…",
        clearSelection: "चयन हटाएँ",
        columnAdded: "कॉलम जोड़ा गया",
        columnRenamed: "कॉलम का नाम बदला",
        columnsReordered: "कॉलम का क्रम सहेजा गया",
        cardsSorted: "कार्ड क्रमबद्ध हुए",
        cardsMoved: "{count} कार्ड ले जाए गए",
        cardsRemoved: "{count} कार्ड हटाए गए",
        removeTitle: "यह कार्ड हटाएँ?",
        removeTitleMany: "{count} कार्ड हटाएँ?",
        removeFromBoardHint: "सिर्फ़ इस बोर्ड से हटाएँ। टास्क अपने प्रोजेक्ट और दूसरे बोर्डों पर बना रहेगा।",
        deleteTasks: "टास्क मिटाएँ",
        deleteTasksHint: "टास्क को ही मिटाएँ। यह अपने प्रोजेक्ट और सभी बोर्डों से हट जाएगा।",
        tasksDeleted: "{count} टास्क मिटाए गए",
        tasksDeleteFailed: "{count} टास्क मिटाए नहीं जा सके",
        searchPlaceholder: "बोर्ड का नाम या विवरण खोजें",
        emptyFiltered: "इन फ़िल्टरों से कोई बोर्ड मेल नहीं खाता।",
        cards: "कार्ड",
        allBoards: "सभी बोर्ड",
        withCards: "कार्ड वाले",
        emptyOnly: "खाली बोर्ड",

        background: {
            title: "पृष्ठभूमि",
            hint: "यह बोर्ड खोलने वाले सभी लोग इसे देखते हैं।",
            colors: "रंग",
            image: "छवि",
            uploadImage: "छवि अपलोड करें",
            replaceImage: "छवि बदलें",
            currentImage: "वर्तमान बोर्ड पृष्ठभूमि",
            fileHint: "JPEG, PNG या WebP, अधिकतम 3.5 MB।",
            remove: "पृष्ठभूमि हटाएँ",
            saved: "पृष्ठभूमि अपडेट हुई",
            tooLarge: "यह छवि बहुत बड़ी है। 3.5 MB से छोटी छवि चुनें।",
            notAnImage: "JPEG, PNG या WebP छवि चुनें।",
            uploadFailed: "छवि पढ़ी नहीं जा सकी।",
            colorNames: {
                GRAY: "स्लेटी",
                BLUE: "नीला",
                EMERALD: "हरा",
                AMBER: "अंबर",
                RED: "लाल",
                PURPLE: "बैंगनी",
            },
        },
    },

} as const;
