export const projectsMessages = {
    title: "إدارة المشاريع",
    subtitle: "الأعمال والمهام والوقت والدورات",
    newProject: "مشروع جديد",
    editProject: "تعديل المشروع",
    empty: "لا توجد مشاريع بعد. أنشئ واحدًا لتبدأ تتبّع عمل ما.",
    emptyFiltered: "لا توجد مشاريع تطابق عوامل التصفية هذه.",
    searchPlaceholder: "ابحث بالرمز أو الاسم أو العميل",
    saved: "تم حفظ المشروع",
    saveFailed: "تعذّر حفظ المشروع",
    loadFailed: "تعذّر تحميل المشروع",
    deleted: "تم حذف المشروع",
    deleteFailed: "تعذّر حذف المشروع",
    deleteProject: "حذف المشروع",
    deletePrompt: "حذف {name}؟\n\nتبقى مهامه والساعات المسجّلة عليها — ولا يُحذف سوى المشروع من قوائمك.",

    fields: {
        code: "الرمز",
        codeHint: "تُبنى مفاتيح المهام منه — {code}-14. مقترح من الاسم؛ يمكنك تغييره.",
        codeEditHint: "تغييره يُبقي مفاتيح المهام القديمة صالحة — فهي تُحوَّل إلى الرمز الجديد.",
        name: "الاسم",
        shortName: "الاسم المختصر",
        shortNameHint: "يظهر على بطاقات اللوحة حيث لا يتّسع الاسم الكامل.",
        description: "الوصف",
        customer: "العميل",
        type: "النوع",
        status: "الحالة",
        priority: "الأولوية",
        visibility: "الظهور",
        manager: "المدير",
        branch: "الفرع",
        startDate: "تاريخ البدء",
        targetEndDate: "الانتهاء المستهدف",
        actualEndDate: "الانتهاء الفعلي",
        budget: "الميزانية",
        tasks: "المهام",
        progress: "التقدم",
        actions: "إجراءات",
        project: "المشروع",
        assignee: "المسؤول",
        sprint: "الدورة",
        dueDate: "الاستحقاق",
    },

    status: {
        DRAFT: "مسودة",
        ACTIVE: "نشط",
        ON_HOLD: "معلّق",
        COMPLETED: "مكتمل",
        CANCELLED: "ملغى",
    },

    visibility: {
        PUBLIC: "عام",
        PRIVATE: "خاص",
    },

    visibilityHelp: {
        PUBLIC: "كل من في مساحة العمل ممن يمكنهم رؤية المشاريع.",
        PRIVATE: "فقط الأشخاص المضافون إلى فريق المشروع.",
    },

    priority: {
        LOW: "منخفضة",
        MEDIUM: "متوسطة",
        HIGH: "عالية",
        URGENT: "عاجلة",
    },

    validation: {
        nameRequired: "أعطِ المشروع اسمًا.",
        codeInvalid: "من 2 إلى 12 حرفًا: أحرف كبيرة وأرقام وشرطات، تبدأ بحرف.",
    },

    statusCategory: {
        TODO: "للتنفيذ",
        IN_PROGRESS: "قيد التنفيذ",
        DONE: "منجز",
    },

    tasks: {
        title: "المهام",
        subtitle: "كل مهمة في كل مشروع",
        empty: "لا توجد مهام تطابق عوامل التصفية هذه.",
        searchPlaceholder: "ابحث في عناوين المهام",
        mine: "المسندة إليّ",
        anyone: "أي شخص",
        allProjects: "كل المشاريع",
        anyStatus: "أي حالة",
        anyPriority: "أي أولوية",
        bulkDeletePrompt: "حذف {count, plural, zero {# مهام محددة} one {مهمة واحدة محددة} two {مهمتين محددتين} few {# مهام محددة} many {# مهمة محددة} other {# مهمة محددة}}؟\n\nستختفي من كل قائمة ولوحة ودورة. لا يمكن التراجع عن ذلك.",
        deletedCount: "{count, plural, zero {لم تُحذف مهام} one {حُذفت مهمة واحدة} two {حُذفت مهمتان} few {حُذفت # مهام} many {حُذفت # مهمة} other {حُذفت # مهمة}}",
        deleteFailedCount: "{count, plural, zero {لم يتعذّر حذف أي مهمة} one {تعذّر حذف مهمة واحدة} two {تعذّر حذف مهمتين} few {تعذّر حذف # مهام} many {تعذّر حذف # مهمة} other {تعذّر حذف # مهمة}}",
    },

    tabs: {
        overview: "نظرة عامة",
        tasks: "المهام",
        board: "اللوحة",
        backlog: "قائمة الانتظار",
        team: "الفريق",
        milestones: "المعالم",
        time: "الوقت",
    },

    overview: {
        estimated: "المقدَّر",
        logged: "المسجَّل",
        remaining: "المتبقي",
        complete: "مكتمل",
        noMilestones: "لا توجد معالم بعد.",
        noTeam: "لا أحد في هذا المشروع بعد.",
    },

    team: {
        accessNote: "هذه القائمة تحدد من يمكنه فتح هذا المشروع. إزالة شخص ما تلغي وصوله أيضًا.",
        add: "إضافة عضو",
        role: "الدور",
        MANAGER: "مدير",
        MEMBER: "عضو",
        VIEWER: "مشاهد",
        remove: "إزالة من المشروع",
        added: "تمت الإضافة إلى المشروع",
        addFailed: "تعذّرت إضافة ذلك الشخص",
        removed: "تمت الإزالة من المشروع",
        removeFailed: "تعذّرت إزالة ذلك الشخص",
        person: "الشخص",
        noLogin: "بلا حساب دخول",
        noLoginHint: "يمكن للموظفين بلا حساب في مساحة العمل أن يكونوا في الفريق ويتولّوا مهامًا، لكن لا يمكنهم تسجيل الدخول لرؤيتها.",
    },

    milestones: {
        add: "إضافة معلم",
        name: "اسم المعلم",
        targetDate: "التاريخ المستهدف",
        markComplete: "تعليمه كمكتمل",
        reopen: "إعادة فتحه",
    },


    /**
     * User stories. The requirement in the words of whoever wants it, with the
     * tasks that deliver it filed under it — see `ProjectStoriesCard`.
     */
    stories: {
        title: 'قصص المستخدم',
        add: "قصة مستخدم جديدة",
        edit: 'تعديل القصة',
        empty: 'لا توجد قصص مستخدم بعد. اكتب واحدة لتوضيح ما هو المطلوب من هذا المشروع.',
        code: "معرّف القصة",
        codeHint: "اتركه فارغًا ليُرقَّم تلقائيًا، مثل {example}.",
        codeAuto: "تلقائي",
        codeRequired: "أعطِ القصة معرّفًا.",
        codeNoSpaces: "لا يمكن أن يحتوي معرّف القصة على مسافات.",
        projectRequired: "اختر مشروعًا.",
        pickProject: "اختر مشروعًا",
        asA: 'بصفتي',
        iWant: 'أريد',
        soThat: 'حتى',
        asAPlaceholder: 'صاحب متجر',
        iWantPlaceholder: 'أن أستلم دفعة بِكاش عند الصندوق',
        soThatPlaceholder: 'لا أضطر إلى عدّ الفكة',
        narrative: 'بصفتي {asA}، أريد {iWant} حتى {soThat}.',
        acceptance: 'معايير القبول',
        acceptancePlaceholder: 'ما الذي يجب أن يتحقق قبل اعتبار هذه القصة منتهية.',
        points: 'النقاط',
        pointsHint: 'حجم نسبي لا ساعات — الساعات تبقى على المهام.',
        pointsShort: '{points} نقطة',
        statuses: {
            BACKLOG: 'قائمة الانتظار',
            READY: 'جاهزة',
            IN_PROGRESS: 'قيد التنفيذ',
            DONE: 'منتهية',
        },
        taskProgress: '{done}/{total} مهمة',
        noTasks: 'لا توجد مهام تحت هذه القصة بعد.',
        addTask: 'إضافة مهمة',
        taskPlaceholder: 'ما الذي يجب بناؤه',
        none: 'بلا قصة',
        field: 'قصة المستخدم',
        created: 'تم إنشاء القصة',
        updated: 'تم حفظ القصة',
        deleted: 'تم حذف القصة',
        deleteStory: 'حذف القصة',
        deletePrompt: 'حذف {title}؟\n\nتبقى المهام التابعة لها — تفقد التجميع فقط.',
        saveFailed: 'تعذّر حفظ القصة',
        deleteFailed: 'تعذّر حذف القصة',
        loadFailed: 'تعذّر تحميل قصص المستخدم',
        titleRequired: 'أعطِ القصة عنوانًا.',
    },

    /**
     * The cross-project backlog at /projects/stories. Its own block rather than
     * more keys on `stories`, which belongs to the card on the project page —
     * the same split `tasks` already makes against the per-project task table.
     */
    storyList: {
        title: "قصص المستخدم",
        subtitle: "كل قصة في كل مشروع",
        empty: "لا توجد قصص مستخدم بعد. اكتب واحدة، أو استورد قائمة من جدول بيانات.",
        emptyFiltered: "لا توجد قصص تطابق عوامل التصفية هذه.",
        searchPlaceholder: "ابحث في عناوين القصص أو ما تطلبه",
        allProjects: "كل المشاريع",
        anyStatus: "أي حالة",
        anyPriority: "أي أولوية",
        story: "قصة",
        openInProject: "افتح في مشروعها",
        anyEpic: "أي ملحمة",
        noEpic: "بلا ملحمة",
    },

    /**
     * Epics — the parent of user stories. `epics` is the card and form on the
     * project page; `epicList` is the cross-project list at /projects/epics.
     */
    epics: {
        title: "الملاحم",
        add: "ملحمة جديدة",
        edit: "تعديل الملحمة",
        empty: "لا توجد ملاحم بعد. اجمع قصص المستخدم المترابطة تحت ملحمة واحدة لمتابعة نطاق أكبر.",
        code: "معرّف الملحمة",
        codeHint: "اتركه فارغًا للترقيم التلقائي، مثل {example}.",
        codeAuto: "تلقائي",
        codeRequired: "أعطِ الملحمة معرّفًا.",
        codeNoSpaces: "لا يمكن أن يحتوي معرّف الملحمة على مسافات.",
        projectRequired: "اختر مشروعًا.",
        pickProject: "اختر مشروعًا",
        description: "الوصف",
        descriptionPlaceholder: "ما الذي يقدّمه هذا العمل، ولماذا هو مهم.",
        color: "اللون",
        startDate: "تاريخ البدء",
        targetDate: "التاريخ المستهدف",
        statuses: {
            OPEN: "مفتوحة",
            IN_PROGRESS: "قيد التنفيذ",
            DONE: "منجزة",
            CANCELLED: "ملغاة",
        },
        storyProgress: "{done}/{total} قصص",
        pointsProgress: "{done}/{total} نقاط",
        taskProgress: "{done}/{total} مهام",
        noStories: "لا توجد قصص في هذه الملحمة بعد.",
        addStory: "إضافة قصة",
        storyPlaceholder: "عنوان قصة في هذه الملحمة",
        field: "الملحمة",
        none: "بلا ملحمة",
        created: "تم إنشاء الملحمة",
        updated: "تم حفظ الملحمة",
        deleted: "تم حذف الملحمة",
        deleteEpic: "حذف الملحمة",
        deletePrompt: "حذف {title}؟\n\nتبقى القصص التابعة لها — تفقد التجميع فقط.",
        saveFailed: "تعذّر حفظ الملحمة",
        deleteFailed: "تعذّر حذف الملحمة",
        loadFailed: "تعذّر تحميل الملاحم",
        titleRequired: "أعطِ الملحمة عنوانًا.",
    },

    epicList: {
        title: "الملاحم",
        subtitle: "كل ملحمة في كل مشروع",
        empty: "لا توجد ملاحم بعد. اكتب واحدة، أو استوردها من جدول بيانات.",
        emptyFiltered: "لا توجد ملاحم تطابق عوامل التصفية هذه.",
        searchPlaceholder: "ابحث عن معرّف الملحمة أو عنوانها",
        allProjects: "كل المشاريع",
        anyStatus: "أي حالة",
        anyPriority: "أي أولوية",
        epic: "ملحمة",
        progress: "التقدّم",
        stories: "القصص",
        openInProject: "افتح في مشروعها",
    },

    task: {
        title: "المهمة",
        newTask: "مهمة جديدة",
        editTask: "تعديل المهمة",
        titleField: "العنوان",
        assignee: "المسؤول",
        dueDate: "تاريخ الاستحقاق",
        milestone: "المعلم",
        estimate: "التقدير (ساعة)",
        remaining: "المتبقي (ساعة)",
        logged: "المسجَّل (ساعة)",
        subtasks: "المهام الفرعية",
        noTasks: "لا توجد مهام بعد.",
        deleted: "تم حذف المهمة",
        deleteTask: "حذف المهمة",
        deletePrompt: "حذف {title}؟\n\nستختفي من كل قائمة ولوحة ودورة. لا يمكن التراجع عن ذلك.",
        deleteFailed: "تعذّر حذف المهمة",
        created: "تم إنشاء المهمة",
        updated: "تم تحديث المهمة",
        editTitle: "تعديل العنوان",
        renameFailed: "تعذّرت إعادة تسمية المهمة",
        details: "التفاصيل",
        unassigned: "غير مسندة",
        saveFailed: "تعذّر تحديث المهمة",
        selectProject: "اختر مشروعًا",
        projectRequired: "اختر المشروع الذي تنتمي إليه هذه المهمة.",
        titleRequired: "أعطِ المهمة عنوانًا.",
        noProjects: "أنشئ مشروعًا أولًا — كل مهمة تنتمي إلى مشروع.",
        createFailed: "تعذّر إنشاء المهمة",
        openFull: "فتح الصفحة الكاملة",
        assigneeNoTeam: "لا أحد في فريق هذا المشروع بعد.",
        assigneeAddTeam: "إضافة أعضاء الفريق",
        assigneeLoadFailed: "تعذّر قراءة فريق هذا المشروع.",
    },

    description: {
        title: "الوصف",
        add: "أضف وصفًا…",
        edit: "تعديل الوصف",
        empty: "لا يوجد وصف بعد.",
        placeholder: "كل ما يحتاج من ينفّذ هذا إلى معرفته.",
        saveFailed: "تعذّر حفظ الوصف",
    },

    dates: {
        title: "التواريخ",
        start: "تاريخ البدء",
        due: "تاريخ الاستحقاق",
        inverted: "تاريخ البدء بعد تاريخ الاستحقاق.",
        saveFailed: "تعذّر حفظ التواريخ",
    },

    activity: {
        title: "النشاط",
        comment: "تعليق",
        commentPlaceholder: "أضف تعليقًا…",
        editComment: "تعديل التعليق",
        empty: "لم يحدث شيء هنا بعد.",
        loadFailed: "تعذّر تحميل النشاط.",
        saveFailed: "تعذّر حفظ ذلك",
        someone: "أحدهم",
        watch: "متابعة",
        watching: "قيد المتابعة",
        watchHint: "يُخطَر المتابعون عند نقل هذه المهمة أو إعادة إسنادها أو تلقّيها تعليقًا.",
        none: '—',
        types: {
            CREATED: "أنشأ هذه المهمة",
            RENAMED: "غيّر اسمها من ”{from}“ إلى ”{to}“",
            STATUS_CHANGED: "نقلها من {from} إلى {to}",
            ASSIGNED: "أسندها إلى {to}",
            UNASSIGNED: "أزال المسؤول عنها",
            PRIORITY_CHANGED: "غيّر الأولوية من {from} إلى {to}",
            DATES_CHANGED: "غيّر التواريخ",
            LABELS_CHANGED: "غيّر التسميات ({count} الآن)",
            RE_ESTIMATED: "أعاد التقدير من {from} ساعة إلى {to} ساعة",
        },
    },

    timeTags: {

        title: "وسوم سجل الساعات",

        hint: "تصنّف *الساعة* لا المهمة — \"قابل للفوترة\"، \"دعم\"، \"إعادة عمل\". قائمة منفصلة عن التسميات، ويمكن لتقرير الساعات أن يجمع بها.",

        empty: "لا توجد وسوم بعد.",

        add: "أضِف وسمًا",

        namePlaceholder: "مثال: قابل للفوترة",

        untagged: "أُزيل من {count} من سجلات الساعات.",

    },

    labels: {
        title: "التسميات",
        hint: "مشتركة بين كل المشاريع، لتتمكن قائمة المهام على مستوى مساحة العمل ولوحة الدورة من التصفية بها.",
        empty: "لا توجد تسميات بعد.",
        add: "إضافة تسمية",
        namePlaceholder: "مثل: معطّلة",
        color: "اللون",
        saveFailed: "تعذّر تحديث التسميات",
        untagged: "أُزيلت من {count} مهمة.",
        colors: {
            GRAY: "رمادي",
            BLUE: "أزرق",
            EMERALD: "أخضر",
            AMBER: "كهرماني",
            RED: "أحمر",
            PURPLE: "بنفسجي",
        },
    },

    columns: {
        title: "أعمدة اللوحة",
        subtitle: "أعمدة لوحة هذا المشروع، وكم بطاقة يسع كلٌّ منها.",
        add: "إضافة عمود",
        empty: "لا تحتوي هذه اللوحة على أعمدة بعد.",
        loadFailed: "تعذّر تحميل الأعمدة.",
        wip: 'WIP',
        noLimit: "بلا حد",
        overLimit: "{name} تجاوز حدّه",
        wipHint: "حد العمل الجاري إرشادي — يُعلَّم العمود عند تجاوزه ولا يُمنع أبدًا. وتعديل افتراضيات المستأجر في إعداد المشاريع يغيّر ما تبدأ به المشاريع الجديدة، لا هذه اللوحة.",
    },

    attachments: {
        title: "المرفقات",
        add: "إرفاق ملف",
        empty: "لا توجد مرفقات بعد.",
        loadFailed: "تعذّر تحميل المرفقات.",
        uploadFailed: "تعذّر إرفاق ذلك الملف",
        deleteFile: "إزالة المرفق",
        preview: "معاينة",
        hint: "JPEG أو PNG أو WebP أو PDF، حتى 5 ميغابايت.",
        tooLarge: "حجم ذلك الملف يتجاوز 5 ميغابايت.",
        unsupported: "استخدم JPEG أو PNG أو WebP أو PDF.",
    },

    cover: {
        title: "الغلاف",
        none: "بلا غلاف",
        pick: "لون الغلاف {color}",
    },

    /** The task card's own furniture: its bottom tabs and its sidebar chart. */
    card: {
        tabs: {
            comments: "التعليقات",
            activity: "النشاط",
            time: "سجل الساعات",
            remaining: "سجل الساعات المتبقية",
            attachments: "المرفقات",
        },
        record: "سجل المهمة",
        addChecklistItem: "إضافة عنصر",
        logTime: "تسجيل الوقت",
        spark: {
            title: "المتبقي",
            hover: "مرّر للتفاصيل",
        },
    },

    checklist: {
        title: "قائمة التحقق",
        add: "إضافة عنصر",
        placeholder: "ما الذي يجب إنجازه؟",
        empty: "لا توجد عناصر في قائمة التحقق بعد.",
        progress: "{done} من {total}",
        allDone: "اكتمل كل شيء",
        moveUp: "تحريك لأعلى",
        moveDown: "تحريك لأسفل",
        deleteItem: "حذف العنصر",
        saveFailed: "تعذّر تحديث قائمة التحقق",
    },

    quickAdd: {
        placeholder: "أضف مهمة — العنوان ثم Enter",
        hint: "التفاصيل في السطر نفسه: @شخص  #وسم  !high  ~3h  >الجمعة",
        add: "إضافة",
        more: "حقول أخرى",
        today: "اليوم",
        tomorrow: "غدًا",
        me: "أنا",
        setPriority: "تحديد الأولوية",
        addDescription: "+ إضافة وصف",
        saveAndAdd: "حفظ وإضافة أخرى",
        suggestions: "اقتراحات",
    },

    timer: {
        running: "قيد التشغيل",
        open: "متتبّع الوقت",
        openTracker: "فتح المتتبّع",
        noTask: "بلا مهمة",
        start: "بدء",
        stop: "إيقاف",
        elsewhere: "هناك مؤقّت يعمل بالفعل على مهمة أخرى",
        idleHint: "لا شيء قيد التشغيل. انقر لبدء مؤقّت أو تسجيل الوقت.",
        startedAt: "بدأ في",
        clickToOpen: "انقر لفتح المتتبّع",
    },

    remaining: {
        chart: "الساعات المتبقية عبر الزمن",
        chartNow: "الآن",
        chartUpNote: "ارتفعت — أُضيف عمل",
        history: "سجل الساعات المتبقية",
        notePlaceholder: "مثل: أضاف العميل غرفتين أخريين",
        empty: "لم تُسجَّل تغييرات بعد.",
        by: "بواسطة",
        sources: {
            TASK_CREATED: "أُنشئت المهمة",
            TIME_LOGGED: "سُجّل وقت",
            RE_ESTIMATED: "أُعيد التقدير",
            TASK_COMPLETED: "اكتملت المهمة",
            TASK_REOPENED: "أُعيد فتح المهمة",
            TIME_ENTRY_DELETED: "حُذف تسجيل وقت",
        },
        hint: "الساعات المتبقية هي تقديرك أنت للعمل الباقي — لا التقدير ناقص ما سُجّل. عدّلها حين يتبيّن أن العمل أكبر مما بدا.",
    },

    time: {
        log: "تسجيل وقت",
        hours: "الساعات",
        workDate: "التاريخ",
        note: "ملاحظة",
        remainingAfter: "المتبقي بعد ذلك",
        remainingHint: "القيمة الافتراضية هي ما يتبقى بعد هذه الساعات. غيّرها إن كبر حجم العمل.",
        logHint: "سجّل ساعات، أو راجع ما تبقّى، أو كليهما — نموذج واحد وحفظ واحد.",
        empty: "لم يُسجَّل أي وقت بعد.",
        logged: "تم تسجيل الوقت",
        deleted: "حُذف تسجيل الوقت",
        totalHours: "إجمالي الساعات",
    },

    hourLogs: {
        title: "سجلات الساعات",
        subtitle: "كل ساعة سُجّلت على مهمة، في قائمة واحدة",
        logHours: "تسجيل ساعات",
        openReport: "التقرير",
        searchPlaceholder: "ابحث في عنوان مهمة أو ملاحظة",
        allProjects: "كل المشاريع",
        allPeople: "الجميع",
        mine: "ساعاتي",
        anyTask: "أي مهمة",
        from: "من",
        to: "إلى",
        person: "الشخص",
        unattributed: "غير منسوبة",
        empty: "لم تُسجَّل ساعات في هذه الفترة.",
        loadFailed: "تعذّر تحميل سجلات الساعات.",
        selectProjectFirst: "اختر مشروعًا لتحديد مهمة.",
        selectTask: "اختر مهمة",
        searchTasks: "ابحث عن المهام…",
        loadingTasks: "جارٍ تحميل المهام…",
        trackerNoTasks: "لا توجد مهام مسندة إليك.",
        trackerNoMatches: "لا توجد مهام تطابق هذا البحث.",
        clearTask: "مسح المهمة المحددة",
        scope: "مهام من تُعرض",
        scopeMine: "مهامي",
        scopeEveryone: "الجميع",
        noTasks: "لا توجد مهام في هذا المشروع بعد.",
        logFailed: "تعذّر تسجيل تلك الساعات.",
        editEntry: "تعديل التسجيل",
        updated: "تم تحديث التسجيل",
        updateFailed: "تعذّر تحديث ذلك التسجيل.",
        deleteEntry: "حذف التسجيل",
        deletePrompt: "حذف الـ{hours} ساعة المسجّلة في {date}؟\n\nتعود الساعات إلى المهمة كعمل متبقٍّ.",
        deleteFailed: "تعذّر حذف ذلك التسجيل.",
        rangeInvalid: "يجب أن يكون تاريخ البدء في تاريخ الانتهاء أو قبله.",
        preset7: "آخر 7 أيام",
        preset30: "آخر 30 يومًا",
        presetMonth: "هذا الشهر",
        presetCustom: "فترة مخصصة",

        // Added with the day-grouped rework: the capture bar, the running
        // clock, spans, tags and the folded rows.
        capturePlaceholder: 'على ماذا تعمل؟',
        captureProject: 'المشروع الذي يُسجَّل عليه',
        captureTask: 'المهمة التي تُسجَّل عليها',
        start: 'ابدأ',
        stop: 'أوقف',
        running: 'المؤقّت يعمل',
        discardTimer: 'تجاهل هذا المؤقّت',
        timerMode: 'القياس بمؤقّت',
        manualMode: 'إدخال الساعات يدويًا',
        startTime: 'وقت البدء',
        endTime: 'وقت الانتهاء',
        spanHint: 'اختياري. أدخل الوقتين معًا أو لا شيء. نهاية قبل البداية تعني أن العمل تجاوز منتصف الليل.',
        hoursFromSpan: 'محسوبة من وقتَي البدء والانتهاء.',
        tags: 'الوسوم',
        allTags: 'كل الوسوم',
        noTags: 'لا توجد وسوم بعد. أضِفها من إعدادات المشاريع.',
        today: 'اليوم',
        yesterday: 'أمس',
        total: 'الإجمالي',
        duration: 'المدة',
        partialDay: 'عرض {shown} من {total} ·',
        addDescription: 'أضِف وصفًا',
        logAgain: 'ابدأ مؤقّتًا لهذا مجددًا',
        expandGroup: 'أظهر السجلات خلف هذا الصف',
        collapseGroup: 'أخفِ هذه السجلات',
        pageOf: 'صفحة {page} من {pages}',
        previous: 'السابق',
        next: 'التالي',
        timerStarted: 'بدأ المؤقّت',
        timerStartFailed: 'تعذّر بدء المؤقّت.',
        timerStopFailed: 'تعذّر إيقاف المؤقّت.',
        timerUpdateFailed: 'تعذّر تحديث المؤقّت.',
        timerDiscarded: 'تم تجاهل المؤقّت. لم يُسجَّل شيء.',
        timerStartedAt: 'بدأ في',
        tracker: 'متتبّع الوقت',
        trackerMove: 'تحريك متتبّع الوقت',
        trackerCollapse: 'طيّ متتبّع الوقت',
        trackerExpand: 'توسيع متتبّع الوقت',
        trackerClose: 'إخفاء متتبّع الوقت',
        timerAlreadyRunning: 'هناك مؤقّت يعمل على "{task}". أوقفه أولًا.',
        timerOverlapped: 'هذه الساعات تتداخل مع وقت سجّلته على "{task}".',
        overlapTitle: 'هذه الساعات متداخلة',
        overlapPrompt: 'هل تحتفظ بالسجلَّين رغم ذلك؟',
        overlapKeepBoth: 'احتفظ بالاثنين',

        view: 'العرض',
        viewDay: 'عرض يومي',
        viewList: 'عرض قائمة',
    },

    hourLogReport: {
        title: "تقرير سجل الساعات",
        subtitle: "الساعات المسجّلة في فترة زمنية، ملخّصة",
        groupBy: "التلخيص حسب",
        openLogs: "فتح سجلات الساعات",
        group: {
            task: "المهمة",
            date: "التاريخ",
            week: "الأسبوع",
            month: "الشهر",
            user: "الشخص",
            project: "المشروع",
            tag: "الوسم",
        },
        totalHours: "إجمالي الساعات",
        entries: "التسجيلات",
        days: "أيام العمل",
        people: "الأشخاص",
        tasks: "المهام",
        projects: "المشاريع",
        avgPerDay: "المتوسط اليومي",
        hours: "الساعات",
        share: "الحصة",
        empty: "لم تُسجَّل ساعات في هذه الفترة.",
        loadFailed: "تعذّر تحميل التقرير.",
        rangeHint: "تشمل الإجماليات العمل المؤرَّخ داخل الفترة، أيًّا كان وقت إدخاله.",
        multiTagged: "{count} من السجلات تحمل أكثر من وسم واحد، لذا تتجاوز النسب مجموع 100%.",
    },

    board: {
        title: "اللوحة",
        kanban: "كانبان",
        scrum: "سكرَم",
        allTasks: "كل المهام",
        noSprint: "لا توجد دورة جارية. ابدأ واحدة من الدورات لاستخدام وضع سكرَم.",
        emptyColumn: "لا شيء هنا",
        noMatches: "لا شيء يطابق عوامل التصفية",
        laneUnassigned: "غير مُسنَد",
        laneNoStory: "بلا قصة مستخدم",
        laneCards: "{count} بطاقات",
        laneDone: "{done} من {count} منجزة",
        laneCollapse: "طيّ المسار",
        laneExpand: "توسيع المسار",
        laneOtherProject: "هذه القصة تتبع مشروعًا آخر، فلا يمكن ضم هذه البطاقة إليها",
        laneColumn: "العمود",
        moveFailed: "تعذّر نقل تلك البطاقة",
        createFailed: "تعذّر إنشاء المهمة",
        columnTotal: "ساعة متبقية",
        addCard: "إضافة بطاقة",
        newCardPlaceholder: "ما الذي يجب إنجازه؟",
        assignCardTo: "تعيين البطاقة إلى",
        filters: {
            assignee: "المسؤول",
            priority: "الأولوية",
            due: "الاستحقاق",
            unassigned: "غير مسندة",
            overdue: "متأخرة",
            dueToday: "مستحقة اليوم",
            dueThisWeek: "مستحقة هذا الأسبوع",
            noDueDate: "بلا تاريخ استحقاق",
            label: "التسمية",
            noLabel: "بلا تسمية",
            clear: "مسح",
            showing: "{shown} من {total} بطاقة",
        },
        card: {
            drag: "اسحب للنقل",
            open: "فتح المهمة",
            project: "المشروع",
            overdue: "متأخرة",
            dueToday: "مستحقة اليوم",
            due: "مستحقة في {date}",
            done: "منجزة",
            hasDescription: "لها وصف",
            checklist: "أُنجز {done} من {total} من عناصر قائمة التحقق",
            comments: "{count} تعليق",
            subtasks: "{count} مهمة فرعية",
            unassigned: "غير مسندة",
        },

        // The board's own appearance controls. The board is read very
        // differently by a lead scanning for what is late and by someone
        // working one lane, so the card layout is a setting, not a decision.
        view: {
            title: "المظهر",
            scopeHint: "خاصة بك وحدك على هذا المتصفح — يرى الآخرون اللوحة بطريقتهم.",
            reset: "إعادة الضبط",
            cardSize: "حجم البطاقة",
            comfortable: "مريح",
            compact: "مضغوط",
            columnWidth: "عرض العمود",
            narrow: "ضيق",
            standard: "قياسي",
            wide: "عريض",
            columnColor: "لون العمود",
            tintNone: "بسيط",
            tintCategory: "حسب المرحلة",
            scroll: "التمرير",
            scrollPage: "الصفحة كاملة",
            scrollColumn: "داخل الأعمدة",
            scrollHint: "التمرير داخل الأعمدة يُبقي رأس اللوحة وعنوان كل عمود في مكانه. أما على الهاتف فتُمرَّر الصفحة في الحالتين.",
            swimlanes: "المسارات",
            swimlanesNone: "بلا",
            swimlanesAssignee: "المكلَّف",
            swimlanesStory: "قصة المستخدم",
            swimlanesHint: "يقسم كل عمود إلى صف لكل شخص أو قصة. تنتقل البطاقات بين الأعمدة داخل صفها فقط.",
            motion: "الحركة",
            motionHint: "تستقر البطاقات والأعمدة في أماكنها بهدوء. يُتجاهل ذلك إذا طلب جهازك تقليل الحركة.",
            showOnCards: "العرض على البطاقات",
            fields: {
                cover: "الشريط",
                labels: "التسميات",
                project: "المشروع",
                badges: "الاستحقاق والأولوية",
                details: "التفاصيل",
                assignee: "المسؤول",
            },
        },
    },

    sprint: {
        title: "الدورة",
        sprints: "الدورات",
        newSprint: "دورة جديدة",
        name: "اسم الدورة",
        goal: "الهدف",
        startDate: "تبدأ",
        endDate: "تنتهي",
        start: "بدء الدورة",
        complete: "إنهاء الدورة",
        planned: "مخططة",
        active: "نشطة",
        completed: "مكتملة",
        backlog: "قائمة الانتظار",
        addToSprint: "إضافة إلى الدورة",
        removeFromSprint: "الإعادة إلى قائمة الانتظار",
        committed: "الملتزَم به",
        carriedOver: "أُعيدت {count} مهمة غير منجزة إلى قائمة الانتظار",
        empty: "لا توجد دورات بعد.",
        emptyBacklog: "قائمة الانتظار فارغة.",
        oneActive: "لا يمكن تشغيل أكثر من دورة واحدة في الوقت نفسه عبر مساحة العمل.",
        subtitle: "فترات زمنية محددة تمتد عبر كل المشاريع",
        planning: "خطّط هذه الدورة",
        loadFailed: "تعذّر تحميل الدورات",
        saveFailed: "تعذّر حفظ الدورة",
        created: "تم إنشاء الدورة",
        started: "بدأت الدورة",
        completedMsg: "اكتملت الدورة",
        deleted: "تم حذف الدورة",
        deleteSprint: "حذف الدورة",
        deletePrompt: "حذف {name}؟\n\nتعود مهامها إلى قائمة الانتظار — ولا يُفقد شيء.",
        emptySprint: "لا توجد مهام في هذه الدورة بعد.",
        noProjects: "لا توجد مشاريع بعد",
        tenantHint: "تمتد الدورة عبر مساحة العمل كلها. ولا يمكن تشغيل أكثر من واحدة في الوقت نفسه.",
        searchPlaceholder: "ابحث في اسم الدورة أو هدفها",
        emptyFiltered: "لا توجد دورات تطابق عوامل التصفية هذه.",
        anyStatus: "أي حالة",
        allProjects: "كل المشاريع",
        dates: "التواريخ",
        projects: "المشاريع",
        addWork: "إضافة عمل",
        addWorkTitle: "إضافة إلى {name}",
        tabTasks: "المهام",
        tabStories: "قصص المستخدم",
        searchBacklog: "البحث في قائمة الأعمال",
        emptyStories: "لا توجد قصص مستخدم مفتوحة.",
        openTasks: "{count} مهمة مفتوحة",
        storyHint: "يضيف كل مهمة مفتوحة ضمن القصص المحددة وليست في سبرنت بالفعل.",
        selectedHours: "{count} محدد · {hours} س",
        tasksAdded: "أُضيفت {count} مهمة إلى السبرنت",
        nothingAdded: "لا تحتوي تلك القصص على مهام مفتوحة خارج سبرنت.",
        removed: "أُعيدت إلى قائمة الأعمال",
        colTask: "المهمة",
        colEstimate: "التقدير (س)",
        colSpent: "المستغرق (س)",
        colRemaining: "المتبقي (س)",
        total: "الإجمالي",
        stats: "إحصاءات رئيسية",
        statProgress: "التقدم",
        statTasksDone: "المهام المنجزة",
        statDaysLeft: "أيام العمل المتبقية",
        statVariance: "مقارنة بالمثالي",
        ahead: "متقدم {hours} س",
        behind: "متأخر {hours} س",
        onTrack: "ضمن المسار",
        viewLabel: "العرض",
        viewTable: "جدول",
        viewCards: "بطاقات",
        searchTasks: "ابحث في المهام والقصص والأشخاص والتسميات",
        noMatches: "لا توجد مهام تطابق البحث.",
        noColumnForProject: "لا يحتوي {project} على عمود \"{column}\"، لذا لا يمكن نقل هذه البطاقة إليه.",
        laneLocked: "تنتقل البطاقات بين الأعمدة داخل صفها فقط.",
        everyone: "الجميع",
        info: "السبرنت",
        noGoal: "لم يُحدَّد هدف",
        timeElapsed: "الوقت المنقضي",
        dayOf: "اليوم {day} من {total}",
    },

    burndown: {
        noTarget: "لا يوجد تاريخ انتهاء مستهدف لهذا المشروع، فلا شيء للمقارنة — الخط يبيّن ما جرى.",
        title: "منحنى الإنجاز",
        ideal: "المثالي",
        actual: "المتبقي",
        committed: "الملتزَم به",
        hours: "الساعات",
        noData: "لا توجد لقطات بعد. يمتلئ الرسم البياني بمجرد مرور يوم على الدورة.",
        rebuild: "إعادة البناء من السجل",
        rebuilt: "أُعيد بناء {written} يوم، وأُبقي على {skipped}",
        weekendNote: "يتخطّى الخط المثالي الجمعة والسبت.",
        tenantScope: "كل المشاريع في هذه الدورة",
        hoursUnit: "س",
        tasksUnit: "مهام",
        openTasks: "المهام المفتوحة",
        openTasksRightAxis: "المهام المفتوحة (المحور الأيمن)",
    },

    settings: {
        title: "إعداد المشاريع",
        subtitle: "أنواع المشاريع وأعمدة اللوحة",
        types: "أنواع المشاريع",
        typesHint: "تصنيفات لأعمالك — تركيب، صيانة، تجهيز.",
        columns: "أعمدة اللوحة",
        columnsHint: "الأعمدة التي يبدأ بها أي مشروع جديد. أما المشروع الموجود بالفعل فيحتفظ بقائمته الخاصة — عدّلها من داخل المشروع نفسه. والفئة تحدد ما يُعد منجزًا.",
        category: "التصنيف",
        categories: {
            TODO: "للتنفيذ",
            IN_PROGRESS: "قيد التنفيذ",
            DONE: "منجز",
        },
        isDefault: "تصل المهام الجديدة هنا",
        addType: "إضافة نوع",
        addColumn: "إضافة عمود",
        columnInUse: "انقل المهام خارج هذا العمود أولًا.",
        typeDeactivated: "هذا النوع مستخدَم في {count} مشروع، لذا أُخفي بدل حذفه.",
    },

    boards: {
        title: "اللوحات",
        subtitle: "بطاقات منتقاة من أي مشروع",
        newBoard: "لوحة جديدة",
        name: "الاسم",
        nameRequired: "أعطِ اللوحة اسمًا.",
        description: "الوصف",
        empty: "لا توجد لوحات بعد. أنشئ واحدة وأضف مهامًا من أي مشروع.",
        addTasks: "إضافة مهام",
        addTasksTitle: "إضافة مهام إلى هذه اللوحة",
        removeCard: "إزالة من اللوحة",
        searchTasks: "ابحث في المهام",
        allProjects: "كل المشاريع",
        selectedCount: "{count} محدد",
        noResults: "لا توجد مهام مطابقة",
        loadAll: "تحميل الكل",
        allOnBoard: "كل مهمة مطابقة موجودة بالفعل على هذه اللوحة.",
        loadMore: "تحميل المزيد",
        showingCount: "عرض {shown} من {total}",
        added: "تمت إضافة المهام",
        removed: "أُزيلت البطاقة",
        created: "تم إنشاء اللوحة",
        deleted: "تم حذف اللوحة",
        deleteBoard: "حذف اللوحة",
        deleteConfirm: "حذف {name}؟ لن تتأثر المهام التي عليها.",
        boardSettings: "إعدادات اللوحة",
        backToBoard: "العودة إلى اللوحة",
        unsorted: "غير مصنّفة",
        unsortedHint: "تقع هذه البطاقات في حالة لم تربطها هذه اللوحة بأي عمود.",
        unmappedDrop: "ذلك العمود غير مربوط لمشروع {project}. اربطه من إعدادات اللوحة.",
        columns: "الأعمدة",
        columnName: "اسم العمود",
        category: "التصنيف",
        wipLimit: "حد العمل الجاري",
        wipInvalid: "يجب ألا يقل حد العمل الجاري عن 1.",
        addColumn: "إضافة عمود",
        mappedStatuses: "الحالات المربوطة",
        noMappings: "غير مربوط بأي مشروع على هذه اللوحة",
        createStatus: "إنشاء",
        createStatusHint: "أنشئ الحالة ”{column}“ في {project} واربطها بهذا العمود.",
        currentlyIn: "(حاليًا في {column})",
        loadFailed: "تعذّر تحميل الأعمدة.",
        deleteColumn: "حذف العمود",
        deleteColumnConfirm: "حذف ”{name}“؟ ستنتقل بطاقاته إلى ”غير مصنّفة“، وتزول معه ارتباطات الحالات.",
        moveColumnUp: "تحريك لأعلى",
        moveColumnDown: "تحريك لأسفل",

        searchCards: "بحث في البطاقات",
        dragColumn: "اسحب لإعادة الترتيب",
        columnActions: "إجراءات العمود",
        renameColumn: "إعادة تسمية",
        moveColumnLeft: "تحريك لليسار",
        moveColumnRight: "تحريك لليمين",
        sortCards: "ترتيب البطاقات",
        sortBy: {
            due: "حسب تاريخ الاستحقاق",
            priority: "حسب الأولوية",
            title: "حسب العنوان",
            assignee: "حسب المسؤول",
        },
        moveAllCards: "نقل كل البطاقات إلى",
        selectAllCards: "تحديد كل البطاقات",
        noOtherColumn: "لا يوجد عمود آخر بعد",
        selectCard: "تحديد البطاقة",
        moveSelectedTo: "نقل المحدد إلى…",
        clearSelection: "إلغاء التحديد",
        columnAdded: "تمت إضافة العمود",
        columnRenamed: "تمت إعادة تسمية العمود",
        columnsReordered: "تم حفظ ترتيب الأعمدة",
        cardsSorted: "تم ترتيب البطاقات",
        cardsMoved: "تم نقل {count} بطاقة",
        cardsRemoved: "تمت إزالة {count} بطاقة",
        removeTitle: "إزالة هذه البطاقة؟",
        removeTitleMany: "إزالة {count} بطاقات؟",
        removeFromBoardHint: "إزالتها من هذه اللوحة فقط. تبقى المهمة في مشروعها وعلى اللوحات الأخرى.",
        deleteTasks: "حذف المهمة",
        deleteTasksHint: "حذف المهمة نفسها. تختفي من مشروعها ومن كل اللوحات.",
        tasksDeleted: "تم حذف {count} مهمة",
        tasksDeleteFailed: "تعذّر حذف {count} مهمة",
        searchPlaceholder: "ابحث في اسم اللوحة أو وصفها",
        emptyFiltered: "لا توجد لوحات تطابق عوامل التصفية هذه.",
        cards: "البطاقات",
        allBoards: "كل اللوحات",
        withCards: "التي بها بطاقات",
        emptyOnly: "اللوحات الفارغة",

        background: {
            title: "الخلفية",
            hint: "يراها كل من يفتح هذه اللوحة.",
            colors: "الألوان",
            image: "صورة",
            uploadImage: "رفع صورة",
            replaceImage: "استبدال الصورة",
            currentImage: "خلفية اللوحة الحالية",
            fileHint: "JPEG أو PNG أو WebP، حتى 3.5 ميغابايت.",
            remove: "إزالة الخلفية",
            saved: "تم تحديث الخلفية",
            tooLarge: "الصورة كبيرة جدًا. استخدم صورة أقل من 3.5 ميغابايت.",
            notAnImage: "استخدم صورة JPEG أو PNG أو WebP.",
            uploadFailed: "تعذّرت قراءة الصورة.",
            colorNames: {
                GRAY: "رمادي",
                BLUE: "أزرق",
                EMERALD: "أخضر",
                AMBER: "كهرماني",
                RED: "أحمر",
                PURPLE: "بنفسجي",
            },
        },
    },

} as const;
