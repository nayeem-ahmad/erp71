export const projectsMessages = {
    title: "Projektmanagement",
    subtitle: "Aufträge, Aufgaben, Zeiten und Sprints",
    newProject: "Neues Projekt",
    editProject: "Projekt bearbeiten",
    empty: "Noch keine Projekte. Legen Sie eines an, um einen Auftrag zu verfolgen.",
    emptyFiltered: "Keine Projekte entsprechen diesen Filtern.",
    searchPlaceholder: "Code, Name oder Kunde suchen",
    saved: "Projekt gespeichert",
    saveFailed: "Das Projekt konnte nicht gespeichert werden",
    loadFailed: "Das Projekt konnte nicht geladen werden",
    deleted: "Projekt gelöscht",
    deleteFailed: "Das Projekt konnte nicht gelöscht werden",
    deleteProject: "Projekt löschen",
    deletePrompt: "{name} löschen?\n\nSeine Aufgaben und die darauf gebuchten Stunden bleiben erhalten — nur das Projekt verschwindet aus Ihren Listen.",

    fields: {
        code: "Code",
        codeHint: "Aufgabenschlüssel werden daraus gebildet — {code}-14. Aus dem Namen vorgeschlagen; gern ändern.",
        codeEditHint: "Eine Änderung lässt alte Aufgabenschlüssel weiter funktionieren — sie leiten zum neuen Code um.",
        name: "Name",
        shortName: "Kurzname",
        shortNameHint: "Wird auf Board-Karten angezeigt, wenn der volle Name nicht passt.",
        description: "Beschreibung",
        customer: "Kunde",
        type: "Typ",
        status: "Status",
        priority: "Priorität",
        visibility: "Sichtbarkeit",
        manager: "Leitung",
        branch: "Filiale",
        startDate: "Startdatum",
        targetEndDate: "Zieltermin",
        actualEndDate: "Tatsächliches Ende",
        budget: "Budget",
        tasks: "Aufgaben",
        progress: "Fortschritt",
        actions: "Aktionen",
        project: "Projekt",
        assignee: "Zuständig",
        sprint: "Sprint",
        dueDate: "Fällig",
    },

    status: {
        DRAFT: "Entwurf",
        ACTIVE: "Aktiv",
        ON_HOLD: "Pausiert",
        COMPLETED: "Abgeschlossen",
        CANCELLED: "Abgebrochen",
    },

    visibility: {
        PUBLIC: "Öffentlich",
        PRIVATE: "Privat",
    },

    visibilityHelp: {
        PUBLIC: "Alle im Arbeitsbereich, die Projekte sehen dürfen.",
        PRIVATE: "Nur die zum Projektteam hinzugefügten Personen.",
    },

    priority: {
        LOW: "Niedrig",
        MEDIUM: "Mittel",
        HIGH: "Hoch",
        URGENT: "Dringend",
    },

    validation: {
        nameRequired: "Geben Sie dem Projekt einen Namen.",
        codeInvalid: "2–12 Zeichen: Großbuchstaben, Ziffern und Bindestriche, beginnend mit einem Buchstaben.",
    },

    statusCategory: {
        TODO: "Zu erledigen",
        IN_PROGRESS: "In Arbeit",
        DONE: "Erledigt",
    },

    tasks: {
        title: "Aufgaben",
        subtitle: "Alle Aufgaben aus allen Projekten",
        empty: "Keine Aufgaben entsprechen diesen Filtern.",
        searchPlaceholder: "Aufgabentitel durchsuchen",
        mine: "Mir zugewiesen",
        anyone: "Alle",
        allProjects: "Alle Projekte",
        anyStatus: "Beliebiger Status",
        anyPriority: "Beliebige Priorität",
        bulkDeletePrompt: "{count, plural, one {# ausgewählte Aufgabe} other {# ausgewählte Aufgaben}} löschen?\n\nSie verschwinden aus jeder Liste, jedem Board und jedem Sprint. Das lässt sich nicht rückgängig machen.",
        deletedCount: "{count, plural, one {# Aufgabe gelöscht} other {# Aufgaben gelöscht}}",
        deleteFailedCount: "{count, plural, one {# Aufgabe konnte nicht gelöscht werden} other {# Aufgaben konnten nicht gelöscht werden}}",
    },

    tabs: {
        overview: "Übersicht",
        tasks: "Aufgaben",
        board: "Board",
        backlog: "Backlog",
        team: "Team",
        milestones: "Meilensteine",
        time: "Zeit",
    },

    overview: {
        estimated: "Geschätzt",
        logged: "Gebucht",
        remaining: "Verbleibend",
        complete: "abgeschlossen",
        noMilestones: "Noch keine Meilensteine.",
        noTeam: "Diesem Projekt ist noch niemand zugeordnet.",
    },

    team: {
        accessNote: "Diese Liste bestimmt, wer dieses Projekt öffnen kann. Wer entfernt wird, verliert auch den Zugriff.",
        add: "Mitglied hinzufügen",
        role: "Rolle",
        MANAGER: "Leitung",
        MEMBER: "Mitglied",
        VIEWER: "Betrachter",
        remove: "Aus dem Projekt entfernen",
        added: "Zum Projekt hinzugefügt",
        addFailed: "Diese Person konnte nicht hinzugefügt werden",
        removed: "Aus dem Projekt entfernt",
        removeFailed: "Diese Person konnte nicht entfernt werden",
        person: "Person",
        noLogin: "kein Zugang",
        noLoginHint: "Mitarbeitende ohne Arbeitsbereichskonto können im Team stehen und Aufgaben halten, sich aber nicht anmelden, um sie zu sehen.",
    },

    milestones: {
        add: "Meilenstein hinzufügen",
        name: "Name des Meilensteins",
        targetDate: "Zieldatum",
        markComplete: "Als erledigt markieren",
        reopen: "Wieder öffnen",
    },


    /**
     * User stories. The requirement in the words of whoever wants it, with the
     * tasks that deliver it filed under it — see `ProjectStoriesCard`.
     */
    stories: {
        title: 'User Storys',
        add: "Neue User Story",
        edit: 'Story bearbeiten',
        empty: 'Noch keine User Storys. Schreiben Sie eine, um festzuhalten, was von diesem Projekt verlangt wird.',
        code: "Story-ID",
        codeHint: "Leer lassen für automatische Nummerierung, z. B. {example}.",
        codeAuto: "Automatisch",
        codeRequired: "Geben Sie der Story eine ID.",
        codeNoSpaces: "Eine Story-ID darf keine Leerzeichen enthalten.",
        projectRequired: "Wählen Sie ein Projekt.",
        pickProject: "Projekt wählen",
        asA: 'Als',
        iWant: 'möchte ich',
        soThat: 'damit',
        asAPlaceholder: 'Ladenbesitzer',
        iWantPlaceholder: 'an der Kasse eine bKash-Zahlung annehmen',
        soThatPlaceholder: 'ich kein Wechselgeld zählen muss',
        narrative: 'Als {asA} möchte ich {iWant}, damit {soThat}.',
        acceptance: 'Akzeptanzkriterien',
        acceptancePlaceholder: 'Was zutreffen muss, bevor diese Story fertig ist.',
        points: 'Punkte',
        pointsHint: 'Relative Größe, keine Stunden — Stunden bleiben an den Aufgaben.',
        pointsShort: '{points} Pkt.',
        statuses: {
            BACKLOG: 'Backlog',
            READY: 'Bereit',
            IN_PROGRESS: 'In Arbeit',
            DONE: 'Fertig',
        },
        taskProgress: '{done}/{total} Aufgaben',
        noTasks: 'Noch keine Aufgaben unter dieser Story.',
        addTask: 'Aufgabe hinzufügen',
        taskPlaceholder: 'Was gebaut werden muss',
        none: 'Keine Story',
        field: 'User Story',
        created: 'Story erstellt',
        updated: 'Story gespeichert',
        deleted: 'Story gelöscht',
        deleteStory: 'Story löschen',
        deletePrompt: '{title} löschen?\n\nDie Aufgaben darunter bleiben erhalten — nur die Gruppierung entfällt.',
        saveFailed: 'Story konnte nicht gespeichert werden',
        deleteFailed: 'Story konnte nicht gelöscht werden',
        loadFailed: 'User Storys konnten nicht geladen werden',
        titleRequired: 'Geben Sie der Story einen Titel.',
        statusDerived: "Folgt den Aufgaben, sobald eine begonnen hat.",
    },

    /**
     * The cross-project backlog at /projects/stories. Its own block rather than
     * more keys on `stories`, which belongs to the card on the project page —
     * the same split `tasks` already makes against the per-project task table.
     */
    storyList: {
        title: "User Storys",
        subtitle: "Alle Storys aus allen Projekten",
        empty: "Noch keine User Storys. Schreiben Sie eine oder importieren Sie ein Backlog aus einer Tabelle.",
        emptyFiltered: "Keine Storys entsprechen diesen Filtern.",
        searchPlaceholder: "Story-Titel oder Anforderung durchsuchen",
        allProjects: "Alle Projekte",
        anyStatus: "Beliebiger Status",
        anyPriority: "Beliebige Priorität",
        story: "Story",
        openInProject: "Im Projekt öffnen",
        anyEpic: "Beliebiges Epic",
        noEpic: "Kein Epic",
    },

    /**
     * Epics — the parent of user stories. `epics` is the card and form on the
     * project page; `epicList` is the cross-project list at /projects/epics.
     */
    epics: {
        title: "Epics",
        add: "Neues Epic",
        edit: "Epic bearbeiten",
        empty: "Noch keine Epics. Fassen Sie zusammengehörige User Storys in einem Epic zusammen, um größeren Umfang zu verfolgen.",
        code: "Epic-ID",
        codeHint: "Leer lassen für automatische Nummerierung, z. B. {example}.",
        codeAuto: "Automatisch",
        codeRequired: "Geben Sie dem Epic eine ID.",
        codeNoSpaces: "Eine Epic-ID darf keine Leerzeichen enthalten.",
        projectRequired: "Wählen Sie ein Projekt.",
        pickProject: "Projekt wählen",
        description: "Beschreibung",
        descriptionPlaceholder: "Was dieses Arbeitspaket liefert und warum es wichtig ist.",
        color: "Farbe",
        startDate: "Startdatum",
        targetDate: "Zieldatum",
        statuses: {
            OPEN: "Offen",
            IN_PROGRESS: "In Arbeit",
            DONE: "Erledigt",
            CANCELLED: "Abgebrochen",
        },
        storyProgress: "{done}/{total} Storys",
        pointsProgress: "{done}/{total} Pkt.",
        taskProgress: "{done}/{total} Aufgaben",
        noStories: "Noch keine Storys in diesem Epic.",
        addStory: "Story hinzufügen",
        storyPlaceholder: "Titel einer Story in diesem Epic",
        field: "Epic",
        none: "Kein Epic",
        created: "Epic erstellt",
        updated: "Epic gespeichert",
        deleted: "Epic gelöscht",
        deleteEpic: "Epic löschen",
        deletePrompt: "{title} löschen?\n\nDie Storys darin bleiben erhalten — sie verlieren nur die Gruppierung.",
        saveFailed: "Epic konnte nicht gespeichert werden",
        deleteFailed: "Epic konnte nicht gelöscht werden",
        loadFailed: "Epics konnten nicht geladen werden",
        titleRequired: "Geben Sie dem Epic einen Titel.",
    },

    /** The per-project tree at /projects/[id]/backlog. */
    backlog: {
        title: "Backlog",
        subtitle: "Epics, User Stories und ihre Aufgaben in einem Baum",
        open: "Backlog",
        searchPlaceholder: "IDs, Titel oder Zuständige suchen",
        hideDone: "Erledigte ausblenden",
        expand: "Aufklappen",
        expandEpics: "Nur Epics",
        expandStories: "Epics und Stories",
        expandAll: "Alles",
        noEpic: "Kein Epic",
        unplanned: "Aufgaben ohne Story",
        empty: "Dieser Backlog ist noch leer. Beginnen Sie mit einem Epic oder schreiben Sie direkt eine Story.",
        emptyFiltered: "Nichts entspricht diesen Filtern.",
        addStory: "Story hinzufügen",
        addTask: "Aufgabe hinzufügen",
        subtasks: "{count} Unteraufgaben",
        hours: "{logged}/{estimate} h",
        unassigned: "Nicht zugewiesen",
        expandRow: "Aufklappen",
        collapseRow: "Zuklappen",
        loadFailed: "Backlog konnte nicht geladen werden",
        createFailed: "Konnte nicht hinzugefügt werden",
    },

    epicList: {
        title: "Epics",
        subtitle: "Alle Epics aus allen Projekten",
        empty: "Noch keine Epics. Legen Sie eins an oder importieren Sie sie aus einer Tabelle.",
        emptyFiltered: "Keine Epics entsprechen diesen Filtern.",
        searchPlaceholder: "Epic-ID oder Titel suchen",
        allProjects: "Alle Projekte",
        anyStatus: "Beliebiger Status",
        anyPriority: "Beliebige Priorität",
        epic: "Epic",
        progress: "Fortschritt",
        stories: "Storys",
        openInProject: "Im Projekt öffnen",
    },

    task: {
        title: "Aufgabe",
        newTask: "Neue Aufgabe",
        editTask: "Aufgabe bearbeiten",
        titleField: "Titel",
        assignee: "Zuständig",
        dueDate: "Fälligkeitsdatum",
        milestone: "Meilenstein",
        estimate: "Schätzung (h)",
        remaining: "Verbleibend (h)",
        logged: "Gebucht (h)",
        subtasks: "Teilaufgaben",
        noTasks: "Noch keine Aufgaben.",
        deleted: "Aufgabe gelöscht",
        deleteTask: "Aufgabe löschen",
        deletePrompt: "{title} löschen?\n\nSie verschwindet aus jeder Liste, jedem Board und jedem Sprint. Das lässt sich nicht rückgängig machen.",
        deleteFailed: "Die Aufgabe konnte nicht gelöscht werden",
        created: "Aufgabe erstellt",
        updated: "Aufgabe aktualisiert",
        editTitle: "Titel bearbeiten",
        renameFailed: "Die Aufgabe konnte nicht umbenannt werden",
        details: "Details",
        unassigned: "Nicht zugewiesen",
        saveFailed: "Die Aufgabe konnte nicht aktualisiert werden",
        selectProject: "Projekt auswählen",
        projectRequired: "Wählen Sie das Projekt, zu dem diese Aufgabe gehört.",
        titleRequired: "Geben Sie der Aufgabe einen Titel.",
        noProjects: "Legen Sie zuerst ein Projekt an — jede Aufgabe gehört zu einem.",
        createFailed: "Die Aufgabe konnte nicht erstellt werden",
        openFull: "Ganze Seite öffnen",
        assigneeNoTeam: "Diesem Projekt ist noch niemand zugeordnet.",
        assigneeAddTeam: "Teammitglieder hinzufügen",
        assigneeLoadFailed: "Das Team dieses Projekts konnte nicht geladen werden.",
    },

    description: {
        title: "Beschreibung",
        add: "Beschreibung hinzufügen…",
        edit: "Beschreibung bearbeiten",
        empty: "Noch keine Beschreibung.",
        placeholder: "Alles, was die ausführende Person wissen muss.",
        saveFailed: "Die Beschreibung konnte nicht gespeichert werden",
    },

    dates: {
        title: "Termine",
        start: "Startdatum",
        due: "Fälligkeitsdatum",
        inverted: "Der Start liegt nach dem Fälligkeitsdatum.",
        saveFailed: "Die Termine konnten nicht gespeichert werden",
    },

    activity: {
        title: "Aktivität",
        comment: "Kommentar",
        commentPlaceholder: "Kommentar hinzufügen…",
        editComment: "Kommentar bearbeiten",
        empty: "Hier ist noch nichts passiert.",
        loadFailed: "Die Aktivität konnte nicht geladen werden.",
        saveFailed: "Das konnte nicht gespeichert werden",
        someone: "Jemand",
        watch: "Beobachten",
        watching: "Wird beobachtet",
        watchHint: "Beobachtende werden benachrichtigt, wenn diese Aufgabe verschoben, neu zugewiesen oder kommentiert wird.",
        none: '—',
        types: {
            CREATED: "hat diese Aufgabe erstellt",
            RENAMED: "hat sie von „{from}“ in „{to}“ umbenannt",
            STATUS_CHANGED: "hat sie von {from} nach {to} verschoben",
            ASSIGNED: "hat sie {to} zugewiesen",
            UNASSIGNED: "hat die Zuweisung entfernt",
            PRIORITY_CHANGED: "hat die Priorität von {from} auf {to} geändert",
            DATES_CHANGED: "hat die Termine geändert",
            LABELS_CHANGED: "hat die Labels geändert (jetzt {count})",
            RE_ESTIMATED: "hat von {from} h auf {to} h neu geschätzt",
        },
    },

    timeTags: {

        title: "Stunden-Tags",

        hint: "Klassifizieren eine *Stunde*, keine Aufgabe — „Abrechenbar“, „Support“, „Nacharbeit“. Eine eigene Liste neben den Labels, und der Stundenbericht kann danach summieren.",

        empty: "Noch keine Tags.",

        add: "Tag hinzufügen",

        namePlaceholder: "z. B. Abrechenbar",

        untagged: "Von {count} Stundeneinträgen entfernt.",

    },

    labels: {
        title: "Labels",
        hint: "Projektübergreifend geteilt, damit die arbeitsbereichsweite Aufgabenliste und das Sprint-Board danach filtern können.",
        empty: "Noch keine Labels.",
        add: "Label hinzufügen",
        namePlaceholder: "z. B. Blockiert",
        color: "Farbe",
        saveFailed: "Die Labels konnten nicht aktualisiert werden",
        untagged: "Von {count} Aufgabe(n) entfernt.",
        colors: {
            GRAY: "Grau",
            BLUE: "Blau",
            EMERALD: "Grün",
            AMBER: "Bernstein",
            RED: "Rot",
            PURPLE: "Violett",
        },
    },

    columns: {
        title: "Board-Spalten",
        subtitle: "Die Spalten auf dem Board dieses Projekts und wie viele Karten jede aufnehmen darf.",
        add: "Spalte hinzufügen",
        empty: "Dieses Board hat noch keine Spalten.",
        loadFailed: "Die Spalten konnten nicht geladen werden.",
        wip: 'WIP',
        noLimit: "Keines",
        overLimit: "{name} liegt über dem Limit",
        wipHint: "Ein WIP-Limit ist eine Empfehlung — die Spalte wird bei Überschreitung markiert, nie blockiert. Änderungen an den Mandanten-Standardwerten in der Projekteinrichtung betreffen neue Projekte, nicht dieses Board.",
    },

    attachments: {
        title: "Anhänge",
        add: "Datei anhängen",
        empty: "Noch nichts angehängt.",
        loadFailed: "Die Anhänge konnten nicht geladen werden.",
        uploadFailed: "Diese Datei konnte nicht angehängt werden",
        deleteFile: "Anhang entfernen",
        preview: "Vorschau",
        hint: "JPEG, PNG, WebP oder PDF, bis 5 MB.",
        tooLarge: "Diese Datei ist größer als 5 MB.",
        unsupported: "Verwenden Sie JPEG, PNG, WebP oder PDF.",
    },

    cover: {
        title: "Titelbild",
        none: "Kein Titelbild",
        pick: "Titelfarbe {color}",
    },

    /** The task card's own furniture: its bottom tabs and its sidebar chart. */
    card: {
        tabs: {
            comments: "Kommentare",
            activity: "Aktivität",
            time: "Stundenprotokoll",
            remaining: "Verlauf der Reststunden",
            attachments: "Anhänge",
        },
        record: "Aufgabenverlauf",
        addChecklistItem: "Eintrag hinzufügen",
        logTime: "Zeit erfassen",
        spark: {
            title: "Rest",
            hover: "für Details bewegen",
        },
    },

    checklist: {
        title: "Checkliste",
        add: "Eintrag hinzufügen",
        placeholder: "Was ist zu tun?",
        empty: "Noch keine Checklisteneinträge.",
        progress: "{done} von {total}",
        allDone: "Alles erledigt",
        moveUp: "Nach oben",
        moveDown: "Nach unten",
        deleteItem: "Eintrag löschen",
        saveFailed: "Die Checkliste konnte nicht aktualisiert werden",
    },

    quickAdd: {
        placeholder: "Aufgabe hinzufügen — Titel, dann Enter",
        hint: "Details in derselben Zeile: @person  #label  !high  ~3h  >freitag",
        add: "Hinzufügen",
        more: "Weitere Felder",
        today: "heute",
        tomorrow: "morgen",
        me: "Ich",
        setPriority: "Priorität setzen",
        addDescription: "+ Beschreibung hinzufügen",
        saveAndAdd: "Speichern und weitere",
        suggestions: "Vorschläge",
    },

    timer: {
        running: "Läuft",
        open: "Zeiterfassung",
        openTracker: "Tracker öffnen",
        noTask: "Keine Aufgabe",
        start: "Start",
        stop: "Stopp",
        elsewhere: "Für eine andere Aufgabe läuft bereits ein Timer",
        idleHint: "Nichts läuft. Klicken, um einen Timer zu starten oder Zeit zu erfassen.",
        startedAt: "Gestartet um",
        clickToOpen: "Klicken, um den Tracker zu öffnen",
    },

    remaining: {
        chart: "Reststunden im Zeitverlauf",
        chartNow: "jetzt",
        chartUpNote: "Gestiegen — Arbeit ergänzt",
        history: "Verlauf der Reststunden",
        notePlaceholder: "z. B. Kunde hat zwei weitere Räume ergänzt",
        empty: "Noch keine Änderungen erfasst.",
        by: "von",
        sources: {
            TASK_CREATED: "Aufgabe erstellt",
            TIME_LOGGED: "Zeit gebucht",
            RE_ESTIMATED: "Neu geschätzt",
            TASK_COMPLETED: "Aufgabe abgeschlossen",
            TASK_REOPENED: "Aufgabe wieder geöffnet",
            TIME_ENTRY_DELETED: "Zeitbuchung gelöscht",
        },
        hint: "Die Reststunden sind Ihre eigene Schätzung der verbleibenden Arbeit — nicht die Schätzung abzüglich der gebuchten Zeit. Passen Sie sie an, wenn der Auftrag größer ausfällt als gedacht.",
    },

    time: {
        log: "Zeit buchen",
        hours: "Stunden",
        workDate: "Datum",
        note: "Notiz",
        remainingAfter: "Rest danach",
        remainingHint: "Voreingestellt auf das, was nach diesen Stunden bleibt. Ändern Sie es, wenn die Arbeit gewachsen ist.",
        logHint: "Stunden buchen, den Rest anpassen oder beides — ein Formular, ein Speichern.",
        empty: "Noch keine Zeit gebucht.",
        logged: "Zeit gebucht",
        deleted: "Zeitbuchung gelöscht",
        totalHours: "Stunden gesamt",
    },

    hourLogs: {
        title: "Stundenbuchungen",
        subtitle: "Jede auf eine Aufgabe gebuchte Stunde in einer Liste",
        logHours: "Stunden buchen",
        openReport: "Bericht",
        searchPlaceholder: "Aufgabentitel oder Notiz suchen",
        allProjects: "Alle Projekte",
        allPeople: "Alle",
        mine: "Meine Stunden",
        anyTask: "Beliebige Aufgabe",
        from: "Von",
        to: "Bis",
        person: "Person",
        unattributed: "Ohne Zuordnung",
        empty: "In diesem Zeitraum wurden keine Stunden gebucht.",
        loadFailed: "Die Stundenbuchungen konnten nicht geladen werden.",
        selectProjectFirst: "Wählen Sie ein Projekt, um eine Aufgabe auszuwählen.",
        selectTask: "Aufgabe auswählen",
        searchTasks: "Aufgaben suchen…",
        loadingTasks: "Aufgaben werden geladen…",
        trackerNoTasks: "Ihnen sind keine Aufgaben zugewiesen.",
        trackerNoMatches: "Keine Aufgaben passen zu dieser Suche.",
        clearTask: "Ausgewählte Aufgabe entfernen",
        scope: "Wessen Aufgaben angezeigt werden",
        scopeMine: "Meine",
        scopeEveryone: "Alle",
        noTasks: "Dieses Projekt hat noch keine Aufgaben.",
        logFailed: "Diese Stunden konnten nicht gebucht werden.",
        editEntry: "Buchung bearbeiten",
        updated: "Buchung aktualisiert",
        updateFailed: "Diese Buchung konnte nicht aktualisiert werden.",
        deleteEntry: "Buchung löschen",
        deletePrompt: "Die am {date} gebuchten {hours} h löschen?\n\nDie Stunden fließen als Restarbeit auf die Aufgabe zurück.",
        deleteFailed: "Diese Buchung konnte nicht gelöscht werden.",
        rangeInvalid: "Das Startdatum muss auf oder vor dem Enddatum liegen.",
        preset7: "Letzte 7 Tage",
        preset30: "Letzte 30 Tage",
        presetMonth: "Dieser Monat",
        presetCustom: "Eigener Zeitraum",

        // Added with the day-grouped rework: the capture bar, the running
        // clock, spans, tags and the folded rows.
        capturePlaceholder: 'Woran arbeitest du gerade?',
        captureProject: 'Projekt, auf das gebucht wird',
        captureTask: 'Aufgabe, auf die gebucht wird',
        start: 'Start',
        stop: 'Stopp',
        running: 'Timer läuft',
        discardTimer: 'Diesen Timer verwerfen',
        timerMode: 'Mit Timer erfassen',
        manualMode: 'Stunden von Hand eintragen',
        startTime: 'Startzeit',
        endTime: 'Endzeit',
        spanHint: 'Optional. Beide angeben oder keine. Ein Ende vor dem Start heißt: über Mitternacht hinaus.',
        hoursFromSpan: 'Aus Start- und Endzeit berechnet.',
        tags: 'Tags',
        allTags: 'Alle Tags',
        noTags: 'Noch keine Tags. Lege sie in den Projekteinstellungen an.',
        today: 'Heute',
        yesterday: 'Gestern',
        total: 'Gesamt',
        duration: 'Dauer',
        partialDay: '{shown} von {total} angezeigt ·',
        addDescription: 'Beschreibung hinzufügen',
        logAgain: 'Timer dafür erneut starten',
        expandGroup: 'Einträge hinter dieser Zeile zeigen',
        collapseGroup: 'Einträge einklappen',
        pageOf: 'Seite {page} von {pages}',
        previous: 'Zurück',
        next: 'Weiter',
        timerStarted: 'Timer gestartet',
        timerStartFailed: 'Timer konnte nicht gestartet werden.',
        timerStopFailed: 'Timer konnte nicht gestoppt werden.',
        timerUpdateFailed: 'Timer konnte nicht aktualisiert werden.',
        timerDiscarded: 'Timer verworfen. Es wurde nichts erfasst.',
        timerStartedAt: 'Gestartet um',
        tracker: 'Zeiterfassung',
        trackerMove: 'Zeiterfassung verschieben',
        trackerCollapse: 'Zeiterfassung einklappen',
        trackerExpand: 'Zeiterfassung ausklappen',
        trackerClose: 'Zeiterfassung ausblenden',
        timerAlreadyRunning: 'Für „{task}“ läuft bereits ein Timer. Stoppe ihn zuerst.',
        timerOverlapped: 'Diese Stunden überschneiden sich mit bereits erfasster Zeit auf „{task}“.',
        overlapTitle: 'Diese Stunden überschneiden sich',
        overlapPrompt: 'Beide Einträge trotzdem behalten?',
        overlapKeepBoth: 'Beide behalten',

        view: 'Ansicht',
        viewDay: 'Tagesansicht',
        viewList: 'Listenansicht',
    },

    hourLogReport: {
        title: "Stundenbericht",
        subtitle: "Gebuchte Stunden in einem Zeitraum, zusammengefasst",
        groupBy: "Zusammenfassen nach",
        openLogs: "Stundenbuchungen öffnen",
        group: {
            task: "Aufgabe",
            date: "Datum",
            week: "Woche",
            month: "Monat",
            user: "Person",
            project: "Projekt",
            tag: "Tag",
        },
        totalHours: "Stunden gesamt",
        entries: "Buchungen",
        days: "Arbeitstage",
        people: "Personen",
        tasks: "Aufgaben",
        projects: "Projekte",
        avgPerDay: "Durchschnitt pro Tag",
        hours: "Stunden",
        share: "Anteil",
        empty: "In diesem Zeitraum wurden keine Stunden gebucht.",
        loadFailed: "Der Bericht konnte nicht geladen werden.",
        rangeHint: "Die Summen umfassen Arbeit, die im Zeitraum datiert ist, unabhängig davon, wann sie erfasst wurde.",
        multiTagged: "{count} Einträge tragen mehr als ein Tag, daher ergeben die Anteile über 100 %.",
    },

    board: {
        title: "Board",
        kanban: "Kanban",
        scrum: "Scrum",
        allTasks: "Alle Aufgaben",
        noSprint: "Es läuft kein Sprint. Starten Sie einen unter Sprints, um den Scrum-Modus zu nutzen.",
        emptyColumn: "Nichts vorhanden",
        noMatches: "Nichts entspricht den Filtern",
        laneUnassigned: "Nicht zugewiesen",
        laneNoStory: "Keine User Story",
        laneCards: "{count} Karten",
        laneDone: "{done}/{count} erledigt",
        laneCollapse: "Zeile einklappen",
        laneExpand: "Zeile ausklappen",
        laneOtherProject: "Diese Story gehört zu einem anderen Projekt, daher kann diese Karte ihr nicht beitreten",
        laneColumn: "Spalte",
        moveFailed: "Diese Karte konnte nicht verschoben werden",
        createFailed: "Die Aufgabe konnte nicht erstellt werden",
        columnTotal: "h übrig",
        addCard: "Karte hinzufügen",
        newCardPlaceholder: "Was ist zu tun?",
        assignCardTo: "Karte zuweisen an",
        filters: {
            assignee: "Zuständig",
            priority: "Priorität",
            due: "Fällig",
            unassigned: "Nicht zugewiesen",
            overdue: "Überfällig",
            dueToday: "Heute fällig",
            dueThisWeek: "Diese Woche fällig",
            noDueDate: "Kein Fälligkeitsdatum",
            label: "Label",
            noLabel: "Kein Label",
            clear: "Zurücksetzen",
            showing: "{shown} von {total} Karten",
        },
        card: {
            drag: "Zum Verschieben ziehen",
            open: "Aufgabe öffnen",
            project: "Projekt",
            overdue: "Überfällig",
            dueToday: "Heute fällig",
            due: "Fällig am {date}",
            done: "Erledigt",
            hasDescription: "Hat eine Beschreibung",
            checklist: "{done}/{total} Checklisteneinträge erledigt",
            comments: "{count} Kommentar(e)",
            subtasks: "{count} Teilaufgabe(n)",
            unassigned: "Nicht zugewiesen",
        },

        // The board's own appearance controls. The board is read very
        // differently by a lead scanning for what is late and by someone
        // working one lane, so the card layout is a setting, not a decision.
        view: {
            title: "Darstellung",
            scopeHint: "Gilt nur für Sie in diesem Browser — alle anderen sehen das Board nach ihren eigenen Einstellungen.",
            reset: "Zurücksetzen",
            cardSize: "Kartengröße",
            comfortable: "Komfortabel",
            compact: "Kompakt",
            columnWidth: "Spaltenbreite",
            narrow: "Schmal",
            standard: "Standard",
            wide: "Breit",
            columnColor: "Spaltenfarbe",
            tintNone: "Schlicht",
            tintCategory: "Nach Phase",
            scroll: "Scrollen",
            scrollPage: "Ganze Seite",
            scrollColumn: "In den Spalten",
            scrollHint: "In den Spalten bleiben die Kopfzeile des Boards und jede Spaltenüberschrift stehen. Auf dem Telefon scrollt in beiden Fällen die Seite.",
            swimlanes: "Swimlanes",
            swimlanesNone: "Keine",
            swimlanesAssignee: "Zuständig",
            swimlanesStory: "User Story",
            swimlanesHint: "Teilt jede Spalte in eine Zeile pro Person oder Story. Karten wandern innerhalb ihrer eigenen Zeile zwischen den Spalten.",
            motion: "Bewegung",
            motionHint: "Karten und Spalten gleiten an ihren Platz. Wird ignoriert, wenn Ihr Gerät weniger Bewegung anfordert.",
            showOnCards: "Auf Karten anzeigen",
            fields: {
                cover: "Farbstreifen",
                labels: "Labels",
                project: "Projekt",
                badges: "Fälligkeit & Priorität",
                details: "Details",
                assignee: "Zuständig",
            },
        },
    },

    sprint: {
        title: "Sprint",
        sprints: "Sprints",
        newSprint: "Neuer Sprint",
        name: "Sprintname",
        goal: "Ziel",
        startDate: "Beginnt",
        endDate: "Endet",
        start: "Sprint starten",
        complete: "Sprint abschließen",
        planned: "Geplant",
        active: "Aktiv",
        completed: "Abgeschlossen",
        backlog: "Backlog",
        addToSprint: "Zum Sprint hinzufügen",
        removeFromSprint: "Zurück ins Backlog",
        committed: "Zugesagt",
        carriedOver: "{count} unerledigte Aufgabe(n) ins Backlog zurückgestellt",
        empty: "Noch keine Sprints.",
        emptyBacklog: "Das Backlog ist leer.",
        oneActive: "Im gesamten Arbeitsbereich kann immer nur ein Sprint laufen.",
        subtitle: "Zeitfenster, die alle Projekte umfassen",
        planning: "Diesen Sprint planen",
        loadFailed: "Die Sprints konnten nicht geladen werden",
        saveFailed: "Der Sprint konnte nicht gespeichert werden",
        created: "Sprint erstellt",
        started: "Sprint gestartet",
        completedMsg: "Sprint abgeschlossen",
        deleted: "Sprint gelöscht",
        deleteSprint: "Sprint löschen",
        deletePrompt: "{name} löschen?\n\nSeine Aufgaben gehen ins Backlog zurück — nichts geht verloren.",
        emptySprint: "In diesem Sprint sind noch keine Aufgaben.",
        noProjects: "Noch keine Projekte",
        tenantHint: "Ein Sprint umfasst den gesamten Arbeitsbereich. Es kann immer nur einer laufen.",
        searchPlaceholder: "Sprintname oder Ziel suchen",
        emptyFiltered: "Keine Sprints entsprechen diesen Filtern.",
        anyStatus: "Beliebiger Status",
        allProjects: "Alle Projekte",
        dates: "Termine",
        projects: "Projekte",
        addWork: "Arbeit hinzufügen",
        addWorkTitle: "Zu {name} hinzufügen",
        tabTasks: "Aufgaben",
        tabStories: "User Stories",
        searchBacklog: "Backlog durchsuchen",
        emptyStories: "Keine offenen User Stories.",
        openTasks: "{count} offene Aufgabe(n)",
        storyHint: "Fügt jede offene Aufgabe der gewählten Stories hinzu, die noch in keinem Sprint ist.",
        selectedHours: "{count} ausgewählt · {hours} h",
        tasksAdded: "{count} Aufgabe(n) zum Sprint hinzugefügt",
        nothingAdded: "Diese Stories haben keine offenen Aufgaben außerhalb eines Sprints.",
        removed: "Zurück ins Backlog",
        colTask: "Aufgabe",
        colEstimate: "Schätzung (h)",
        colSpent: "Aufgewendet (h)",
        colRemaining: "Verbleibend (h)",
        total: "Summe",
        stats: "Kennzahlen",
        statProgress: "Fortschritt",
        statTasksDone: "Erledigte Aufgaben",
        statDaysLeft: "Verbleibende Arbeitstage",
        statVariance: "Gegenüber Ideal",
        ahead: "{hours} h voraus",
        behind: "{hours} h im Rückstand",
        onTrack: "Im Plan",
        viewLabel: "Ansicht",
        viewTable: "Tabelle",
        viewCards: "Karten",
        searchTasks: "Aufgaben, Stories, Personen, Labels suchen",
        noMatches: "Keine Aufgaben passen zur Suche.",
        noColumnForProject: "{project} hat keine Spalte „{column}“, daher kann diese Karte nicht dorthin.",
        laneLocked: "Karten wechseln die Spalte nur innerhalb ihrer eigenen Zeile.",
        everyone: "Alle",
        info: "Sprint",
        noGoal: "Kein Ziel festgelegt",
        timeElapsed: "Verstrichene Zeit",
        dayOf: "Tag {day} von {total}",
        focusLane: "Nur diese Bahn zeigen",
        expandAllLanes: "Alle Bahnen aufklappen",
        lanesCollapsed: "{count} eingeklappt",
    },

    burndown: {
        noTarget: "Dieses Projekt hat kein Zieldatum, also gibt es keinen Vergleichsmaßstab — die Linie zeigt den Verlauf, nicht den Abgleich.",
        title: "Burndown",
        ideal: "Ideal",
        actual: "Verbleibend",
        committed: "Zugesagt",
        hours: "Stunden",
        noData: "Noch keine Momentaufnahmen. Das Diagramm füllt sich, sobald der Sprint einen Tag gelaufen ist.",
        rebuild: "Aus Historie neu aufbauen",
        rebuilt: "{written} Tag(e) neu aufgebaut, {skipped} beibehalten",
        weekendNote: "Die Ideallinie überspringt Fr./Sa.",
        tenantScope: "Alle Projekte in diesem Sprint",
        hoursUnit: "h",
        tasksUnit: "Aufgaben",
        openTasks: "Offene Aufgaben",
        openTasksRightAxis: "Offene Aufgaben (rechte Achse)",
    },

    settings: {
        title: "Projekteinrichtung",
        subtitle: "Projekttypen und Board-Spalten",
        types: "Projekttypen",
        typesHint: "Kategorien für Ihre Aufträge — Installation, Wartung, Ausbau.",
        columns: "Board-Spalten",
        columnsHint: "Die Spalten, mit denen ein neues Projekt startet. Ein bereits bestehendes Projekt behält seine eigene Liste — die wird im Projekt selbst geändert. Die Kategorie entscheidet, was als erledigt gilt.",
        category: "Kategorie",
        categories: {
            TODO: "Zu erledigen",
            IN_PROGRESS: "In Arbeit",
            DONE: "Erledigt",
        },
        isDefault: "Neue Aufgaben landen hier",
        addType: "Typ hinzufügen",
        addColumn: "Spalte hinzufügen",
        columnInUse: "Verschieben Sie zuerst die Aufgaben aus dieser Spalte.",
        typeDeactivated: "Dieser Typ wird von {count} Projekt(en) verwendet und wurde daher ausgeblendet statt gelöscht.",
    },

    boards: {
        title: "Boards",
        subtitle: "Handverlesene Karten aus beliebigen Projekten",
        newBoard: "Neues Board",
        name: "Name",
        nameRequired: "Geben Sie dem Board einen Namen.",
        description: "Beschreibung",
        empty: "Noch keine Boards. Legen Sie eines an und fügen Sie Aufgaben aus beliebigen Projekten hinzu.",
        addTasks: "Aufgaben hinzufügen",
        addTasksTitle: "Aufgaben zu diesem Board hinzufügen",
        removeCard: "Vom Board entfernen",
        searchTasks: "Aufgaben suchen",
        allProjects: "Alle Projekte",
        selectedCount: "{count} ausgewählt",
        noResults: "Keine passenden Aufgaben",
        loadAll: "Alle laden",
        allOnBoard: "Alle passenden Aufgaben sind bereits auf diesem Board.",
        loadMore: "Mehr laden",
        showingCount: "{shown} von {total} werden angezeigt",
        added: "Aufgaben hinzugefügt",
        removed: "Karte entfernt",
        created: "Board erstellt",
        deleted: "Board gelöscht",
        deleteBoard: "Board löschen",
        deleteConfirm: "{name} löschen? Die Aufgaben darauf bleiben unberührt.",
        boardSettings: "Board-Einstellungen",
        backToBoard: "Zurück zum Board",
        unsorted: "Unsortiert",
        unsortedHint: "Diese Karten stehen in einem Status, den dieses Board keiner Spalte zugeordnet hat.",
        unmappedDrop: "Diese Spalte ist für {project} nicht zugeordnet. Ordnen Sie sie in den Board-Einstellungen zu.",
        columns: "Spalten",
        columnName: "Spaltenname",
        category: "Kategorie",
        wipLimit: "WIP-Limit",
        wipInvalid: "Das WIP-Limit muss mindestens 1 betragen.",
        addColumn: "Spalte hinzufügen",
        mappedStatuses: "Zugeordnete Status",
        noMappings: "Für kein Projekt auf diesem Board zugeordnet",
        createStatus: "Anlegen",
        createStatusHint: "Den Status „{column}“ in {project} anlegen und dieser Spalte zuordnen.",
        currentlyIn: "(derzeit in {column})",
        loadFailed: "Die Spalten konnten nicht geladen werden.",
        deleteColumn: "Spalte löschen",
        deleteColumnConfirm: "„{name}“ löschen? Ihre Karten fallen auf Unsortiert zurück, und ihre Statuszuordnungen entfallen ebenfalls.",
        moveColumnUp: "Nach oben",
        moveColumnDown: "Nach unten",

        searchCards: "Karten durchsuchen",
        dragColumn: "Zum Umsortieren ziehen",
        columnActions: "Spaltenaktionen",
        renameColumn: "Umbenennen",
        moveColumnLeft: "Nach links",
        moveColumnRight: "Nach rechts",
        sortCards: "Karten sortieren",
        sortBy: {
            due: "Nach Fälligkeit",
            priority: "Nach Priorität",
            title: "Nach Titel",
            assignee: "Nach zuständiger Person",
        },
        moveAllCards: "Alle Karten verschieben nach",
        selectAllCards: "Alle Karten auswählen",
        noOtherColumn: "Noch keine andere Spalte",
        selectCard: "Karte auswählen",
        moveSelectedTo: "Auswahl verschieben nach …",
        clearSelection: "Auswahl aufheben",
        columnAdded: "Spalte hinzugefügt",
        columnRenamed: "Spalte umbenannt",
        columnsReordered: "Spaltenreihenfolge gespeichert",
        cardsSorted: "Karten sortiert",
        cardsMoved: "{count} Karte(n) verschoben",
        cardsRemoved: "{count} Karte(n) entfernt",
        removeTitle: "Diese Karte entfernen?",
        removeTitleMany: "{count} Karten entfernen?",
        removeFromBoardHint: "Nur von diesem Board nehmen. Die Aufgabe bleibt im Projekt und auf anderen Boards.",
        deleteTasks: "Aufgabe(n) löschen",
        deleteTasksHint: "Die Aufgabe selbst löschen. Sie verschwindet aus dem Projekt und von allen Boards.",
        tasksDeleted: "{count} Aufgabe(n) gelöscht",
        tasksDeleteFailed: "{count} Aufgabe(n) konnten nicht gelöscht werden",
        searchPlaceholder: "Boardname oder Beschreibung suchen",
        emptyFiltered: "Keine Boards entsprechen diesen Filtern.",
        cards: "Karten",
        allBoards: "Alle Boards",
        withCards: "Mit Karten",
        emptyOnly: "Leere Boards",

        background: {
            title: "Hintergrund",
            hint: "Alle, die dieses Board öffnen, sehen ihn.",
            colors: "Farben",
            image: "Bild",
            uploadImage: "Bild hochladen",
            replaceImage: "Bild ersetzen",
            currentImage: "Aktueller Board-Hintergrund",
            fileHint: "JPEG, PNG oder WebP, bis 3,5 MB.",
            remove: "Hintergrund entfernen",
            saved: "Hintergrund aktualisiert",
            tooLarge: "Das Bild ist zu groß. Verwende eines unter 3,5 MB.",
            notAnImage: "Verwende ein JPEG-, PNG- oder WebP-Bild.",
            uploadFailed: "Das Bild konnte nicht gelesen werden.",
            colorNames: {
                GRAY: "Grau",
                BLUE: "Blau",
                EMERALD: "Grün",
                AMBER: "Bernstein",
                RED: "Rot",
                PURPLE: "Violett",
            },
        },
    },

} as const;
