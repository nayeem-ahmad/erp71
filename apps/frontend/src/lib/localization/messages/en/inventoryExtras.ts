export const inventoryExtrasMessages = {
    "inventoryCategories": {
        "title": "Product Categories",
        "subtitle": "Organize products into groups and subgroups for cleaner inventory browsing and reporting",
        "newGroup": "New Group",
        "newSubgroup": "New Subgroup",
        "productGroups": "Product Groups",
        "productSubgroups": "Product Subgroups",
        "emptyGroups": "No product groups found",
        "emptySubgroups": "No product subgroups found",
        "searchGroups": "Search groups...",
        "searchSubgroups": "Search subgroups...",
        "noDescription": "No description",
        "featured": "Featured",
        "deleteGroupConfirm": "Delete this group?",
        "deleteSubgroupConfirm": "Delete this subgroup?",
        "deleteGroupFailed": "Failed to delete product group.",
        "deleteSubgroupFailed": "Failed to delete product subgroup.",
        "editProductGroup": "Edit Product Group",
        "newProductGroup": "New Product Group",
        "editProductSubgroup": "Edit Product Subgroup",
        "newProductSubgroup": "New Product Subgroup",
        "createGroup": "Create Group",
        "updateGroup": "Update Group",
        "createSubgroup": "Create Subgroup",
        "updateSubgroup": "Update Subgroup",
        "saveGroupFailed": "Failed to save product group.",
        "saveSubgroupFailed": "Failed to save product subgroup.",
        "categoryImageUrl": "Category Image URL",
        "parentGroup": "Parent Group",
        "columns": {
            "group": "Group",
            "subgroup": "Subgroup",
            "featured": "Featured",
            "subgroups": "Subgroups",
            "products": "Products"
        },
        "saving": "Saving..."
    },
    "inventoryDemands": {
        "title": "Product Demands",
        "subtitle": "Ask head office for stock a branch has run out of, and track what was approved",
        "newDemand": "New Demand",
        "newTitle": "New Product Demand",
        "editTitle": "Edit Product Demand",
        "formHint": "List what the branch needs. Nothing moves stock — an approved demand is met by a transfer or a purchase.",
        "loadFailed": "Failed to load product demands.",
        "empty": "No product demands raised yet",
        "searchPlaceholder": "Search demands...",
        "filters": {
            "allStatuses": "All Statuses",
            "allPriorities": "All Priorities",
            "allWarehouses": "All Warehouses",
            "mineOnly": "Only mine"
        },
        "form": {
            "warehouse": "Deliver To",
            "selectWarehouse": "Select warehouse",
            "selectProduct": "Select product",
            "lineNote": "Line Note",
            "lineNotePlaceholder": "e.g. shelf empty since Friday",
            "notesPlaceholder": "Anything the approver should know",
            "addLine": "Add Product",
            "removeLine": "Remove product line",
            "saveDraft": "Save as Draft",
            "submitForApproval": "Submit for Approval"
        },
        "columns": {
            "demandNumber": "Demand #",
            "warehouse": "Deliver To",
            "priority": "Priority",
            "requested": "Requested",
            "approved": "Approved",
            "neededBy": "Needed By",
            "created": "Raised"
        },
        "statuses": {
            "draft": "Draft",
            "submitted": "Submitted",
            "approved": "Approved",
            "rejected": "Rejected",
            "fulfilled": "Fulfilled",
            "cancelled": "Cancelled"
        },
        "priorities": {
            "low": "Low",
            "normal": "Normal",
            "high": "High",
            "urgent": "Urgent"
        },
        "actions": {
            "submit": "Submit for Approval",
            "cancel": "Withdraw",
            "approve": "Approve",
            "reject": "Reject",
            "markFulfilled": "Mark Fulfilled"
        },
        "detail": {
            "reviewedOn": "Reviewed",
            "reviewNote": "Approver Note",
            "reviewNotePlaceholder": "Why the quantities were changed, or why it was rejected",
            "fulfilmentNote": "Fulfilment Note",
            "fulfilmentNotePlaceholder": "e.g. TRF-00012",
            "fulfilmentHint": "Record the transfer or purchase that met this demand."
        },
        "toasts": {
            "saved": "Demand saved.",
            "submitted": "Demand submitted for approval.",
            "cancelled": "Demand withdrawn.",
            "approved": "Demand approved.",
            "rejected": "Demand rejected.",
            "fulfilled": "Demand marked fulfilled."
        },
        "errors": {
            "warehouseRequired": "Choose the warehouse the goods are needed at.",
            "noLines": "Add at least one product.",
            "badQuantity": "Every line needs a whole quantity of 1 or more.",
            "duplicateProduct": "The same product is listed twice. Combine the lines.",
            "saveFailed": "Failed to save the demand.",
            "actionFailed": "That action could not be completed."
        }
    },
    "inventoryTransfers": {
        "title": "Warehouse Transfers",
        "subtitle": "Send stock between warehouses and receive it only when it physically arrives",
        "newTransfer": "New Transfer",
        "allStatuses": "All Statuses",
        "allSources": "All Sources",
        "allDestinations": "All Destinations",
        "allProducts": "All Products",
        "sourceWarehouse": "Source Warehouse",
        "destinationWarehouse": "Destination Warehouse",
        "initialStatus": "Initial Status",
        "sendNow": "Send Now",
        "saveAsDraft": "Save as Draft",
        "selectSource": "Select source",
        "selectDestination": "Select destination",
        "selectProduct": "Select product",
        "addLine": "Add Line",
        "createTransfer": "Create Transfer",
        "transferCreated": "Transfer created.",
        "createFailed": "Failed to create transfer.",
        "emptyMessage": "No warehouse transfers recorded yet",
        "searchPlaceholder": "Search transfers...",
        "unknownBranch": "Unassigned branch",
        "allScopes": "All Branches",
        "crossBranchOnly": "Between branches",
        "withinBranchOnly": "Within one branch",
        "withinBranch": "Same branch",
        "crossBranchNotice": "This transfer crosses branches, so it waits for approval — no stock leaves the source until someone with approval rights releases it.",
        "columns": {
            "transferNumber": "Transfer #",
            "source": "Source",
            "destination": "Destination",
            "branch": "Branch",
            "outstanding": "Outstanding",
            "created": "Created",
            "voucher": "Voucher"
        },
        "statuses": {
            "draft": "Draft",
            "sent": "Sent",
            "partiallyReceived": "Partially Received",
            "received": "Received",
            "pendingApproval": "Pending Approval",
            "rejected": "Rejected"
        }
    },
    "inventoryShrinkage": {
        "title": "Inventory Shrinkage",
        "subtitle": "Record damaged, lost, or stolen stock separately from sales and returns",
        "newEntry": "New Shrinkage Entry",
        "selectWarehouse": "Select warehouse",
        "selectReason": "Select reason",
        "postShrinkage": "Post Shrinkage",
        "posted": "Shrinkage posted.",
        "postFailed": "Failed to post shrinkage.",
        "emptyMessage": "No shrinkage records yet",
        "searchPlaceholder": "Search shrinkage records...",
        "columns": {
            "direction": "Type",
            "reference": "Reference",
            "warehouse": "Warehouse",
            "reason": "Reason",
            "totalQty": "Total Qty",
            "created": "Created",
            "voucher": "Voucher"
        },
        "directionLabel": "Entry type",
        "directionLoss": "Stock lost (shrinkage)",
        "directionFound": "Stock found (extra)",
        "newFoundEntry": "New Found-Stock Entry",
        "postFound": "Post Found Stock",
        "foundPosted": "Found stock posted.",
        "postFoundFailed": "Failed to post found stock.",
        "noFoundReasons": "No found-stock reasons yet. Add one under Inventory Settings.",
        "warehouseRequired": "Select a warehouse.",
        "reasonRequired": "Select a reason.",
        "noteRequired": "A note is required.",
        "noteHelp": "Say what happened and who checked — this note is the only evidence behind the entry.",
        "productRequired": "Select a product.",
        "quantityRequired": "Enter a quantity of at least 1.",
        "filterLabel": "Filter by entry type",
        "filterAll": "All entries",
        "addLine": "Add Line",
        "remove": "Remove",
        "notes": "Notes",
        "selectProduct": "Select product"
    },
    "inventoryStockTakes": {
        "title": "Stock Takes",
        "subtitle": "Open warehouse counting sessions, capture discrepancies, and post reconciliations",
        "newSession": "New Stock-Take Session",
        "sessionNotes": "Session notes",
        "startImmediately": "Start counting immediately",
        "createSession": "Create Session",
        "sessionCreated": "Stock-take session created.",
        "createFailed": "Failed to create stock-take session.",
        "sessionsTitle": "Stock-Take Sessions",
        "emptyMessage": "No stock-take sessions recorded yet",
        "searchPlaceholder": "Search stock takes...",
        "selectWarehouse": "Select warehouse",
        "warehouse": "Warehouse",
        "columns": {
            "sessionNumber": "Session #",
            "warehouse": "Warehouse",
            "status": "Status",
            "progress": "Progress",
            "discrepancies": "Discrepancies",
            "created": "Created",
            "actions": "Actions",
            "open": "Open"
        }
    },
    "inventoryLedger": {
        "title": "Inventory Ledger",
        "subtitle": "Review stock movement history across products and warehouses",
        "allWarehouses": "All Warehouses",
        "movementType": "Movement Type",
        "movementPlaceholder": "e.g. SALE",
        "stockLedger": "Stock Ledger",
        "emptyMessage": "No inventory movements recorded yet",
        "searchPlaceholder": "Search by product, warehouse, or movement...",
        "warehouseLabel": "Warehouse",
        "columns": {
            "timestamp": "Timestamp",
            "product": "Product",
            "warehouse": "Warehouse",
            "movement": "Movement",
            "delta": "Delta",
            "balance": "Balance",
            "reference": "Reference"
        }
    },
    "inventoryLabels": {
        "title": "Print Labels",
        "searchPlaceholder": "Search products…",
        "selectAll": "Select all",
        "selected": "{count} selected",
        "clear": "Clear",
        "loading": "Loading…",
        "noProducts": "No products found",
        "noSku": "No SKU",
        "copiesPerLabel": "Copies per label",
        "printSelected": "Print Selected ({count})",
        "previewEmpty": "Select products on the left to preview labels",
        previewTitle: "{count, plural, one {Preview — # label} other {Preview — # labels}}",
        "upload": "Upload",
        "print": "Print"
    },
    "warehousesPage": {
        "title": "Warehouses",
        "subtitle": "Stock locations per branch, with defaults and status",
        "newWarehouse": "New Warehouse",
        "editWarehouse": "Edit Warehouse",
        "branch": "Branch",
        "code": "Code",
        "status": "Status",
        "emptyMessage": "No warehouses yet",
        "searchPlaceholder": "Search warehouses...",
        "selectBranch": "Select a branch",
        "nameRequired": "Name is required",
        "nameDuplicate": "A warehouse with this name already exists in this branch",
        "branchRequired": "Branch is required",
        "codeHint": "Leave blank to generate one automatically",
        "manageWarehouses": "Manage warehouses"
    },
    "inventorySettings": {
        "title": "Inventory Settings",
        "subtitle": "Configure warehouse defaults, alert thresholds, and adjustment reason catalogs",
        "loading": "Loading inventory settings…",
        "warehouseDefaults": "Warehouse Defaults",
        "warehouses": "Warehouses",
        "alertRules": "Alert Rules",
        "reasonCatalog": "Reason Catalog",
        "productCreationWarehouse": "Product Creation Warehouse",
        "purchaseReceiptWarehouse": "Purchase Receipt Warehouse",
        "salesIssueWarehouse": "Sales Issue Warehouse",
        "shrinkageWarehouse": "Shrinkage Warehouse",
        "transferSourceWarehouse": "Transfer Source Warehouse",
        "transferDestinationWarehouse": "Transfer Destination Warehouse",
        "selectWarehouse": "Select warehouse",
        "storeId": "Store ID",
        "warehouseName": "Warehouse name",
        "optionalCode": "Optional code",
        "addWarehouse": "Add Warehouse",
        "default": "Default",
        "secondary": "Secondary",
        "active": "Active",
        "inactive": "Inactive",
        "makeDefault": "Make Default",
        "deactivate": "Deactivate",
        "activate": "Activate",
        "defaultReorderLevel": "Default Reorder Level",
        "defaultSafetyStock": "Default Safety Stock",
        "defaultLeadTimeDays": "Default Lead Time Days",
        "discrepancyApprovalThreshold": "Discrepancy Approval Threshold",
        "costingMethod": "Costing Method",
        "costingMethodLabel": "Cost basis for sales",
        "costingWeightedAverage": "Weighted average",
        "costingLatestCost": "Latest cost price",
        "costingWeightedAverageHelp": "Each sale is costed at the running average of what the stock on hand was actually bought for.",
        "costingLatestCostHelp": "Each sale is costed at the most recent cost price on the product price list.",
        "costingMethodNote": "Applies to new sales only. Sales already recorded keep the cost they were saved with.",
        "stockPolicy": "Stock Policy",
        "allowNegativeStock": "Allow selling without stock",
        "allowNegativeStockOnHelp": "Sales, POS and storefront orders can be saved for more than the quantity on hand. The stock balance goes negative until a purchase catches up.",
        "allowNegativeStockOffHelp": "A sale is refused when a line asks for more than the quantity on hand.",
        "allowNegativeStockNote": "Applies to selling only. Transfers, stock takes, shrinkage and manufacturing always need the stock on hand.",
        "shrinkage": "Shrinkage",
        "found": "Found (Extra Stock)",
        "discrepancy": "Discrepancy",
        "reasonCode": "Reason code",
        "reasonLabel": "Reason label",
        "addReason": "Add Reason",
        "system": "System",
        "settingsUpdated": "Inventory settings updated.",
        "saveFailed": "Failed to save inventory settings.",
        "warehouseCreated": "Warehouse created.",
        "warehouseCreateFailed": "Failed to create warehouse.",
        "warehouseUpdated": "Warehouse updated.",
        "warehouseUpdateFailed": "Failed to update warehouse.",
        "warehouseDefaultUpdated": "Warehouse default updated.",
        "warehouseDefaultFailed": "Failed to set warehouse default.",
        "reasonCreated": "Inventory reason created.",
        "reasonCreateFailed": "Failed to create inventory reason.",
        "reasonUpdated": "Inventory reason updated.",
        "reasonUpdateFailed": "Failed to update inventory reason."
    },
    "inventoryReports": {
        "reorder": {
            "title": "Reorder Suggestions",
            "subtitle": "Products below reorder thresholds with suggested purchase quantities",
            "subtitlePrioritize": "Prioritize products that are below target stock after accounting for inbound transfers",
            "allWarehouses": "All Warehouses",
            "allBranches": "All Branches",
            "allGroups": "All Groups",
            "allSubgroups": "All Subgroups",
            "uncategorized": "Uncategorized",
            "emptyMessage": "No reorder suggestions right now",
            "columns": {
                "product": "Product",
                "group": "Group",
                "onHand": "On Hand",
                "inTransit": "In Transit",
                "targetStock": "Target Stock",
                "target": "Target",
                "suggestedQty": "Suggested Qty",
                "shortageReason": "Shortage Reason",
                "explanation": "Explanation",
                "configSource": "Config Source",
                "policySource": "Policy Source",
                "leadTimeDays": "Lead Time (Days)"
            }
        },
        "stockOnHand": {
            "title": "Stock on Hand",
            "subtitle": "Quantity per warehouse valued at weighted average purchase cost",
            "allWarehouses": "All Warehouses",
            "allBranches": "All Branches",
            "allGroups": "All Groups",
            "allSubgroups": "All Subgroups",
            "allBrands": "All Brands",
            "showZeroStock": "Include zero-stock products",
            "uncategorized": "Uncategorized",
            "emptyMessage": "No stock on hand for the current filters",
            "closedWarehouse": "{warehouse} (closed)",
            "loadError": "Stock on hand could not be loaded: {message}",
            "loadErrorEmpty": "Stock on hand could not be loaded",
            "noWarehouses": "No warehouses to report on. Stock is counted per warehouse — add one under Inventory → Warehouses.",
            "searchPlaceholder": "Search products...",
            "totalStockValue": "Total Stock Value",
            "totalQuantity": "Total Quantity",
            "productsInStock": "Products In Stock",
            "warehousesCounted": "Warehouses",
            "valuationBasisNote": "Valued at weighted average purchase cost, net of purchase returns. Products never purchased through the ledger fall back to their last recorded cost.",
            "uncostedWarning": "{count} product(s) holding {quantity} unit(s) have no purchase cost on file and are valued at zero — the total below is understated.",
            "costBasis": {
                "WEIGHTED_AVERAGE": "Weighted average",
                "LATEST_COST": "Last recorded cost",
                "UNCOSTED": "No cost on file"
            },
            "columns": {
                "product": "Product",
                "sku": "SKU",
                "group": "Group",
                "brand": "Brand",
                "avgUnitCost": "Avg Unit Cost",
                "costBasis": "Cost Basis",
                "totalQuantity": "Total Qty",
                "totalValue": "Stock Value"
            }
        },
        "productTransactionHistory": {
            "title": "Product Transaction History",
            "subtitle": "Every movement in and out of one product, with its opening quantity and a running balance",
            "productLabel": "Product",
            "selectProduct": "Select a product",
            "changeProduct": "Change product",
            "searchPlaceholder": "Search by name or SKU...",
            "noProductSelected": "Pick a product to see its transaction history.",
            "noMatches": "No products match that search",
            "searching": "Searching...",
            "warehouseLabel": "Warehouse",
            "allWarehouses": "All Warehouses",
            "branchLabel": "Branch",
            "allBranches": "All Branches",
            "fromLabel": "From",
            "toLabel": "To",
            "clearFilters": "Clear filters",
            "openingQuantity": "Opening Qty",
            "totalIn": "Total In",
            "totalOut": "Total Out",
            "closingQuantity": "Closing Qty",
            "openingRow": "Opening balance",
            "openingRowDated": "Opening balance as at {date}",
            "broughtForward": "Balance brought forward",
            "emptyMessage": "No movements for this product in the selected period",
            "loadError": "Transaction history could not be loaded: {message}",
            "loadErrorEmpty": "Transaction history could not be loaded",
            "stockMismatch": "This card closes at {closing} but the warehouse holds {onHand}. Stock set outside the movement ledger - a seeded or imported quantity - is what usually explains the gap.",
            "deletedProduct": "This product has been deleted from the catalogue. Its movement history is kept.",
            "allWarehousesNote": "The balance runs across every warehouse in scope. Pick one warehouse to read its card on its own.",
            "unitsSuffix": "units",
            "columns": {
                "date": "Date",
                "movement": "Movement",
                "reference": "Reference",
                "party": "Party",
                "warehouse": "Warehouse",
                "quantityIn": "In",
                "quantityOut": "Out",
                "balance": "Balance",
                "unitCost": "Unit Cost",
                "value": "Value",
                "note": "Note"
            }
        },
        "valuation": {
            "title": "Inventory Valuation",
            "subtitle": "Stock value by product and warehouse using current on-hand quantities",
            "subtitleMeasure": "Measure on-hand stock value across products, categories, and warehouses",
            "emptyMessage": "No valuation data available",
            "emptyValuation": "No inventory valuation data available",
            "totalValue": "Total Stock Value",
            "totalQuantity": "Total Quantity",
            "productsWithStock": "Products With Stock",
            "averageUnitValue": "Average Unit Value",
            "searchPlaceholder": "Search inventory valuation...",
            "columns": {
                "product": "Product",
                "warehouse": "Warehouse",
                "group": "Group",
                "quantity": "Quantity",
                "unitCost": "Unit Cost",
                "unitValue": "Unit Value",
                "totalValue": "Total Value",
                "stockValue": "Stock Value"
            }
        },
        "shrinkage": {
            "title": "Shrinkage Report",
            "subtitle": "Shrinkage totals by reason, warehouse, and product over a date range",
            "subtitleTrack": "Track inventory loss by warehouse, reason, and estimated value impact",
            "emptyMessage": "No shrinkage in this period",
            "emptyFiltered": "No shrinkage records match the current filters",
            "fromDate": "From",
            "toDate": "To",
            "totalQuantity": "Total Quantity",
            "totalUnitsLost": "Total Units Lost",
            "estimatedValueLost": "Estimated Value Lost",
            "topDriver": "Top Driver",
            "noShrinkageLogged": "No shrinkage logged",
            "direction": "Direction",
            "directionLoss": "Stock lost",
            "directionFound": "Stock found",
            "totalUnitsFound": "Total Units Found",
            "estimatedValueFound": "Estimated Value Found",
            "noFoundLogged": "No found stock logged",
            "emptyFoundMessage": "No found stock in this period",
            "allWarehousesLabel": "All warehouses",
            "allReasons": "All Reasons",
            "shrinkageSummary": "Shrinkage Summary",
            "searchPlaceholder": "Search shrinkage summary...",
            "columns": {
                "reason": "Reason",
                "warehouse": "Warehouse",
                "product": "Product",
                "quantity": "Quantity",
                "quantityFound": "Quantity Found",
                "quantityLost": "Quantity Lost",
                "estimatedValue": "Estimated Value"
            }
        }
    },
    "inventoryTransferDetail": {
        "loading": "Loading transfer…",
        "transferTitle": "Transfer {number}",
        "routeSubtitle": "{source} to {destination} • {status}",
        "sendTransfer": "Send Transfer",
        "receiveStock": "Receive Stock",
        "transferSent": "Transfer sent to destination warehouse.",
        "sendFailed": "Failed to send transfer.",
        "submitForApproval": "Submit for Approval",
        "approveTransfer": "Approve",
        "rejectTransfer": "Reject",
        "transferApproved": "Transfer approved — stock has left the source warehouse.",
        "approveFailed": "Failed to approve transfer.",
        "transferRejected": "Transfer rejected.",
        "rejectFailed": "Failed to reject transfer.",
        "awaitingApprovalNotice": "Waiting for approval. The stock is still at the source branch and will not move until this is approved.",
        "rejectedNotice": "This transfer was rejected, so no stock moved.",
        "rejectionReasonLabel": "Reason for rejection",
        "rejectionReasonPlaceholder": "Tell the requester why, so they know what to change",
        "confirmRejection": "Confirm Rejection",
        "scope": "Scope",
        "crossBranch": "Between branches",
        "withinBranch": "Within one branch",
        "receiptRecorded": "Transfer receipt recorded.",
        "receiveFailed": "Failed to receive transfer.",
        "transferLines": "Transfer Lines",
        "sentReceived": "Sent {sent} • Received {received}",
        "outstanding": "Outstanding {count}",
        "statusComplete": "Complete",
        "statusInTransit": "In Transit",
        "statusLabel": "Status",
        "noReceiptAction": "No receipt action available",
        "transferTimeline": "Transfer Timeline",
        "timeline": {
            "created": "Created",
            "awaitingApproval": "Awaiting approval",
            "approved": "Approved",
            "rejected": "Rejected",
            "sent": "Sent",
            "partiallyReceived": "Partially received",
            "completed": "Completed"
        },
        "auditSnapshot": "Audit snapshot",
        "source": "Source",
        "destination": "Destination",
        "outstandingUnits": "Outstanding units"
    },
    "inventoryStockTakeDetail": {
        "loading": "Loading stock-take session…",
        "title": "Stock Take {number}",
        "subtitle": "{warehouse} • {status} • {count} discrepant lines",
        "saveCounts": "Save Counts",
        "moveToReview": "Move to Review",
        "returnToCounting": "Return to Counting",
        "postSession": "Post Session",
        "countsSaved": "Count lines saved.",
        "saveFailed": "Failed to save count lines.",
        "movedToReview": "Session moved to review.",
        "statusUpdateFailed": "Failed to update session status.",
        "posted": "Stock-take posted to inventory.",
        "postFailed": "Failed to post stock-take.",
        "returnFailed": "Failed to return to counting.",
        "approvalThreshold": "Approval Threshold",
        "maxVariance": "Max Variance",
        "netQuantityImpact": "Net Quantity Impact",
        "estimatedValueImpact": "Estimated Value Impact",
        "reviewRequired": "This session exceeds the discrepancy threshold and must be moved to review before posting.",
        "expected": "Expected {qty}",
        "counted": "Counted",
        "noReason": "No reason",
        "variance": "Variance {value}",
        "note": "Note"
    },
    "addProductModal": {
        "editTitle": "Edit Product",
        "addTitle": "Add New Product",
        "productName": "Product Name",
        "productType": "Product Type",
        "typeGoods": "Physical Good",
        "typeService": "Service",
        "typeHint": "Services (printing, binding, transport, ...) aren't stock-tracked.",
        "salePrice": "Sale Price ($)",
        "featuredLabel": "Mark product as Featured (shows in Trending)",
        "initialStock": "Initial Stock Level",
        "warrantyEnabled": "Warranty Enabled",
        "warrantyDuration": "Warranty Duration (Days)",
        "reorderLevel": "Reorder Level",
        "safetyStock": "Safety Stock",
        "leadTimeDays": "Lead Time Days",
        "vatRate": "VAT Rate (%)",
        "vatRateHint": "Blank uses the workspace default. 0 means zero-rated or exempt.",
        "sdRate": "Supplementary Duty (%)",
        "sdRateHint": "Third Schedule goods only. Blank means none.",
        "quantityUnit": "Quantity Unit",
        "brand": "Brand",
        "noBrand": "No Brand",
        "group": "Group",
        "subgroup": "Subgroup",
        "uncategorized": "Uncategorized",
        "savingChanges": "Saving Changes...",
        "creatingProduct": "Creating Product...",
        "confirmAdd": "Confirm & Add Product",
        "useGlobal": "Use global",
        "upload": "Upload",
        "placeholders": {
            "name": "e.g., Wireless Mechanical Keyboard",
            "sku": "WH-KB-1032",
            "price": "120.00",
            "initialStock": "50",
            "warrantyDays": "e.g. 365",
            "vatRate": "e.g. 15",
            "sdRate": "e.g. 25"
        }
    }
} as const;
