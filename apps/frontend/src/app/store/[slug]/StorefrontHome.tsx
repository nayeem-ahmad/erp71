...
import StorefrontEnquiryForm from '@/components/storefront/StorefrontEnquiryForm';
 ...
 *
 * The hero and the four category tiles used to hotlink Unsplash when a shop had
 * uploaded no image of its own, which meant a hardware shop in Mirpur showing a
 * stranger's produce aisle, fetched from a third party on every page load, on
 * the page it was trying to look professional on. A shop's own brand colour is
 * a better answer than somebody else's photograph: it is the shop's, it costs
 * no request, and it cannot 404.
 */
...
export default function StorefrontHome({ slug }: { slug: string }) {
    const { t } = useI18n();
    ...
    const heroImage = data.tenant.storefront_hero_image;
    const heroHeadline = data.tenant.storefront_hero_headline || m.defaultHeadline;
    const theme = storefrontTheme(data.tenant.brand_primary_color);

    // The shop's menu, minus the two destinations the footer's first column
    // already lists — a footer repeating "Home" and "Shop" beside "All products"
    // is noise, and those are the two entries every shop's menu has.
    const footerLinks = (data.menu_links ?? []).filter(
        (link) => link.href !== `/store/${slug}` && link.href !== `/store/${slug}/shop`,
    );
    ...
    return (
        ...
        >
            ...
            <main id="top">
                <section className="relative min-h-[78vh] flex items-center overflow-hidden bg-[color:var(--sf-accent)]">
                    <div className="absolute inset-0">
                        {heroImage && (
                            <img src={heroImage} alt="" className="w-full h-full object-cover opacity-70" />
                        )}
                        {/* Over a photo this is the scrim that keeps the headline
                            readable; with no photo it is a graded wash of the
                            shop's own colour, which is why the section carries
                            the accent as its background rather than gray-900. */}
                                ...
                                {data.categories.map((category) => (
                                    ...
                                    >
                                        {category.image_url ? (
                                            ...
                                            <div className="w-full h-full bg-[color:var(--sf-accent)] transition-transform duration-700 group-hover:scale-110" />
                                        )}
                                            ...
                                            <div className="relative aspect-[4/5] bg-gray-100 overflow-hidden">
                                                ...
                                                <div className="absolute bottom-4 start-4 end-4 translate-y-0 opacity-100 md:translate-y-4 md:opacity-0 md:group-hover:translate-y-0 md:group-hover:opacity-100 transition-all duration-300">
                                                    <button
                                                        ...
                                                        className="w-full bg-[color:var(--sf-accent)] text-[color:var(--sf-accent-foreground)] backdrop-blur font-semibold py-3 rounded-xl transition-opacity hover:opacity-90 disabled:opacity-40"
                                                    >
                ...
                <section id="contact" className="py-20 bg-white">
                    <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
                        <div className="mb-8">
                            <h2 className="text-3xl font-bold tracking-tight">{enquiry.title}</h2>
                            <p className="text-gray-500 mt-2">{enquiry.subtitle}</p>
                        </div>
                        <StorefrontEnquiryForm slug={slug} />
                    </div>
                </section>
            ...
            <footer className="bg-gray-900 text-white pt-20 pb-10">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-16">
                        ...
                        {/*
                          * The shop's own pages, from the menu it built — not the
                          * fixed Contact/Shipping/Returns/FAQ list that used to
                          * stand here with all four entries pointing at
                          * `#contact`, which was this footer. Contact stays,
                          * because the form it points at is real now; the other
                          * three were labels for pages that never existed.
                          */}
                        <div>
                            <h4 className="font-bold text-lg mb-6">{footer.information}</h4>
                            <ul className="space-y-4 text-gray-400">
                                ...
                                {/* Always last, and always present: the enquiry
                                    form is on this page whether or not the shop
                                    has written any pages of its own. */}
                                <li>
                    ...
                    <div className="pt-8 border-t border-gray-800 text-center md:text-start flex flex-col md:flex-row justify-between items-center text-gray-400 text-sm">
                        ...
                        <p className="mt-4 md:mt-0">{formatMessage(footer.poweredBy, { brand: 'ERP71' })}</p>
                    </div>
                </div>
            </footer>
