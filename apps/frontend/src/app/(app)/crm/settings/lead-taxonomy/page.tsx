...
import { PageShell, PageHeader, Button, Input, Field, Select, StatusBadge } from '@/components/ui';
...
export default function LeadTaxonomySettingsPage() {
    const { t } = useI18n();
    const m = t.crm.leadTaxonomy;

    const [kind, setKind] = useState<LeadTaxonomyKind>('sources');
    ...
    const openCreate = () => {
        setNameError(null);
        setEditor({ name: '', score_weight: '5' });
    };
    const openEdit = (row: LeadTaxonomyOption) => {
        setNameError(null);
        setEditor({ id: row.id, name: row.name, score_weight: String(row.score_weight ?? 5) });
    };
    ...
    const save = async () => {
        ...
        setSaving(true);
        try {
            const payload = isSources ? { name, score_weight: weight } : { name };
            if (editor.id) {
    ...
    return (
        <PageShell>
                ...
                actions={
                    canManage ? (
                        <Button onClick={openCreate} icon={<Plus className="w-4 h-4" />}>
                            {isSources ? m.addSource : m.addCategory}
                        </Button>
                    ) : null
                }
            />
            ...
            <div className="flex gap-2" role="tablist">
                {(['sources', 'categories'] as const).map((k) => (
                    <button
                        key={k}
                        type="button"
                        role="tab"
                    ...
                    >
                        {k === 'sources' ? m.tabs.sources : m.tabs.categories}
                    </button>
                ))}
            </div>
                            ...
                            <li key={row.id} className="flex items-center justify-between gap-3 px-3 py-2">
                                <div className="min-w-0">
                                    ...
                                    <p className="mt-0.5 text-xs text-gray-400">
                                        ...
                                        {m.leadsUsing.replace('{count}', String(usage[row.id] ?? 0))}
                                    </p>
                                </div>
                                {canManage && (
            ...
            {editor && (
                <ModalShell onBackdropClick={() => setEditor(null)}>
                    <ModalHeader
                        title={editor.id ? m.editTitle : (isSources ? m.addSource : m.addCategory)}
                ...
                <ModalShell onBackdropClick={() => setDeleting(null)}>
                    ...
                    <div className="space-y-3 p-4">
                        ...
                        {inUseCount > 0 && (
                            <>
                                <p className="text-sm text-gray-700">
                                    {m.delete.inUse.replace('{count}', String(inUseCount))}
                                </p>
