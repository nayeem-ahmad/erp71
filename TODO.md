...
- [x] Fix production login 500 — `generateAuthResponse` queries `Tenant` with all columns; `sms_credits` (added in `20260612010000_add_sms_credits`) was missing from production DB because `preDeployCommand` (`prisma db push`) hadn't applied the migration; fix: committed `packages/database/index.js` to trigger a Render redeploy so `prisma db push` runs and syncs schema — done 2026-06-14
